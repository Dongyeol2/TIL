...
```java
/* 사실상 재귀알고리즘이라기보다는 구현에 가까운 문제 */

class Solution {
    /* 초기 행렬 값 */
    public void createMap(int[][] map, int rows, int columns) {
        int count = 0;
    ...
    /* 행렬 lotate */
    public int lotate(int[][] map, int x1, int y1, int x2, int y2) {
        int startPoint = map[x1][y1];
        int min = startPoint;
    ...
    public int[] solution(int rows, int columns, int[][] queries) {
        ...
        for(int i = 0; i < queries.length; i++) {
            ...
            int x2 = queries[i][2];
            int y2 = queries[i][3];
            answer[i] = lotate(map,x1,y1,x2,y2);
        }
