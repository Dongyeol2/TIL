...
/*
완전탐색
play_time, adv_time, logs 모두 sec로 변환
totalSec 배열에 play_time 시간 동안의 모든 logs들이 들어감
currSum은 totalSec배열을 하나씩 증가시키며 그 구간의 advSec만큼의 logs가 포함되는 모든 sec를 더함
currSum > maxSum이면 currSum = maxSum, 그때의 maxIdx값 구함
maxIdx값을 다시 HH:MM:SS 형식으로 변환하여 return
*/
class Solution {
    /* HH:MM:SS 형식의 time string을 초단위로 계산 */
    public int convert(String time) {
        String[] timeSec = time.split(":");
        return Integer.parseInt(timeSec[0]) * 3600 +
            Integer.parseInt(timeSec[1]) * 60 +
            Integer.parseInt(timeSec[2]);
    }
    
    public String solution(String play_time, String adv_time, String[] logs) {
        /* play_time의 초단위 계산, adv_tiem의 초단위 계산 */ 
        int playSec = convert(play_time);
        int advSec = convert(adv_time);
        
        /* totalSec배열 사이즈는 최대값 100시간을 초단위로 계산 */
        int[] totalSec = new int[100*3600];
        /* logs배열에서 모든 log들의 동영상 시청 시작/끝을 초단위로 계산 후 totalSec에 1씩 증가 */
        for(String log : logs) {
        ...
        /* 초기 currSum은 play_time이 00:00:00일때 부터 시작 */
        long currSum = 0;
        ...
        long maxSum = currSum;
        int maxIdx = 0;
        /* adv_time을 play_time동안에 1초씩 증가시키며 가장 많이 시청한 maxSum 구하고 그떄의 maxIdx 계산*/
