...
- **Toaster.yang(OpenYuma sample yang model)을 이용한 yang-explorer test**
  ...
  위의 그림과 같이 Custom RPC 선택 후 RUN을 클릭하게 되면 client(yang-explorer)는 server(OpenYuma)로 \<get\>으로 가지고 오겠다는 메시지를 전달하며(위의 예에서는 toasterManufacturer, toasterModelNumber, toasterStatus) Server에서는 관련 메시지에 해당하는 데이터를 client에게 전달하게 된다. (위의 예에서는 toasterManufacturer = Acme, Inc / toasterModelNumber = Super Toastamatic 2000 / toasterStatus = up)
