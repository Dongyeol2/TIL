...
10. [Cisco YANG SUITE (Yang application) - ★★★★](#10-cisco-yang-suite-yang-application)<br>10.1 [Docker Installation](#101-docker-installation)<br>10.2 [Python Virtualenv Installation](#102-python-virtualenv-installation)<br>10.3 [Yang Suite installation](#103-yang-suite-installation)<br>
...
OpenYuma파일 $HOME (/home/dylee) 경로로 복사한다.

※ [5.1.1 Searching OpenYuma Roots](#511-searching-openyuma-roots) 참고해보면 $HOME 디렉토리가 첫 검색 루트이기 때문

![image](https://user-images.githubusercontent.com/50439622/102185745-e0d61180-3ef4-11eb-876e-0edcbfeb234b.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<그림 21. OpenYuma $HOME 디렉토리로 복사
...
# 10. Cisco YANG SUITE (Yang application)

yang suite는 Docker 기반의 YANG모델 (RFC 6020, RFC 7950) 및 NETCONF(RFC 6241)와 관련된 도구 세트이다. 

Cisco yang suite document : https://developer.cisco.com/docs/yangsuite/</br>
Cisco yang suite application 사용법 : https://www.youtube.com/watch?v=smrhjL5Ayz0  

virtual box 다운로드 및 버전 참고 : [6. OpenYuma OpenSource Installation](#6-openyuma-opensource-installation)

## 10.1 Docker Installation
...
apt-cache policy docker-ce 명령어 수행 후 Installed:(none) 이라면 아직 도커가 설치되어 있지 않다는 것을 알 수 있음. 아래 명령어 실행후 도커 설치 완료.

```shell
sudo apt install docker-ce
```

docker 정상적으로 실행중인지 확인. Active : active (running) 상태로 표시되면 docker가 실행상태인 것을 알 수 있음.
...
- 172.17.0.1/16 docker0 주소 제거 (ipconfig 통해 확인)

  ```
  sudo ip addr del 172.17.0.1/16 dev docker0
  ```

- 새로 넣어줄 docker0 인터페이스의 네트워크 주소 입력

  ```
  sudo ip addr add 192.168.103.17/24 dev docker0
  ```
...
- /etc/docker/daemon.json 설정
  ...
  /etc/docker/daemon.json 파일 생성 이후 아래 설정 값 입력

  ```
  {
    "bip" : "192.168.103.17/24"
  }
  ```
...
## 10.2 Python Virtualenv Installation

yang suite는 python 3.6, 3.7 또는 3.8 버전 설치 필수.

```
python3 --version 
```

통해 python 버전 확인 3.x 버전 install 되어있으면 10.2 Python Virtualenv Installation skip

1. python 3.6 설치

   ```shell
   sudo apt-get install python3 // python3 설치 
   sudo apt-get update // ubuntu 업데이트 
   sudo apt-get upgrade python3 // python3 업그레이드
   ```
...
4. python의 버전 변경
   ...
   sudo update-alternatives --config python3 // python 버전 변경
   ```
...
5. python 버전 확인
   ...
   두 명령어 모두 "Python 3.6.x" 가 출력되어야 정상.

   기존에 python 2.7 버전이 설치되어 있다면 python --version 명령어 "Python 2.7.x"로 출력될 경우가 있음

   python 2.7 -> python 3.6 변경방법
   ...
   python --version -> python 3.6 버전으로 나와야 yangsuite 설치 가능
   ```
   ...
   ※ "Command "python setup.py egg_info "failed with error code 1 ..." error 발생 시
...
## 10.3 Yang Suite installation

https://github.com/CiscoDevNet/yangsuite README.md 파일 참고
...
2. ~~Generate the certificates for the HTTPS secure connection~~ (※ 21.07.01 이후 자동 스크립트로 대체됨 -> 3번 수행)
   ...
   ※ nginx container와 함께 설치된 SSL/TLS 인증서는 신뢰할 수 있는 인증기관을 사용하여 얻거나 생성해야 한다. cisco yang suite 에서는 편의상 gen_test_certs.sh 스크립트 사용하여 자체 서명된 인증서/키를 생성하여 제공한다.

3. Run start_yang_suite.sh
   ...
   start_yang_suite.sh 실행 시 username, password, e-mail address 등의 인증서 세부정보를 설정하도록 요청한다.
   빨간색 표시된 영역에 인증서 세부 정보를 입력해야한다. 아래의 예시와 같이 수행

   ![image](https://user-images.githubusercontent.com/50439622/125216846-545cc780-e2fa-11eb-9b26-8a7e6e009597.png) <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 38. startup_yang_suite 실행                          
...
4. Run docker-compose
   ...
   ※ docker-compose up 처음 수행시 빌드를 하는데 시간이 다소 소요됨. (20-30분) ubuntu 네트워크에 브리지  고정 ip address 설정 이전에 docker-compose up 빌드 먼저 수행되어야 함. 빌드가 완료 되면 ctrl+c 로 종료 이후에 ubuntu 네트워크 고정 ip 설정. (고정 ip 먼저 설정 이후 빌드 하게 될 경우 ubuntu update 수행 이 안되어 build가 안될 수 있음.)

   ※ sudo docker-compose up 수행 전 dependencies library 설치

   ```shell
   sudo apt-get install git openssh-client iputils-ping python3.6 python3-pip sqlite3 vim
   ```

   ※ docker-compose up 수행 시 'ERROR: Couldn't connect to Docker daemon - you might need to run 'docker-machine start default' error 발생할 경우

   ```shell
   docker-compose --version (build 되어있는지 확인)
   ...
   ※ docker 실행 시 'cannot connect to the docker daemon at unix:///var/run/docker.sock...' error 발생할 경우
...
5. https://localhost 접속

   초기 admin id : admin / password : superuser
