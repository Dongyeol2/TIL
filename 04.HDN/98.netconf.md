...
처음 YumaPro에서 제공하는 .conf 파일은 '-sample'이라는 명칭이 붙어있기 때문에 파일명을 변경할 필요가 있다.
...
OpenYume Development Manual에 대한 자세한 내용은 위의 링크[openyuma-dev-manual.odt](https://github.com/OpenClovis/OpenYuma/blob/master/netconf/doc/yuma_docs/openyuma-dev-manual.odt)를 참조하면 된다. Development Manual에는 NETCONF/YANG 및 netconfd 서버의 소프트웨어 개발 측면에 중점을 두고 있다. SIL(Server Instrumentation Library)에 대한 개념과 각 Source file의 목적을 말하며, module의 역할과 쓰임에 대해서 설명하고 있다.
...
- **$YUMA_HOME Directory**
  ...
  - **$YUMA_HOME**
    ...
    $YUMA_HOME 환경변수에는 현재 OpenYuma 프로젝트 루트 디렉토리의 디렉토리 사양이 포함됨.<br />OpenYuma 소소 트리 내의 'netconf' 디렉토리 경로
    ...
    ```c shell
    setenv YUMA_HOME /home/dylee/Openyuma_master/netconf
    ```

    Bash shell example

    ```bash
    export YUMA_HOME=/home/dylee/Openyuma_master/netconf
    ```
...
SIL은 **Server Instrumentation Library**이다. 여기에는 YANG 모듈 명령문에 정의된 대로 특정 동작을 구현하는 networking 장치에 YANG 콘텐츠(netconfd에서 관리)를 binding 하는 'glue code'가 포함되어 있다. <br/>**서버는 부팅 시 또는 runtime 시 로드되는 YANG파일을 직접 사용하여 모든 NETCONF 콘텐츠, 작업, 알림을 관리**한다.<br/>Callback 함수는 장치 및 데이터 모델 특정 동작을 데이터베이스 객체 및 RPC 작업에 연결하는데 사용된다. **yangdump 프로그램은 특정 YANG 모듈에 대한 initialization, cleanup, 'empty' callback 함수를 생성하는데 사용**된다. **'load'** command 는 YANG 모듈과 SIL을 활성화 하기 위해 사용된다.
...
## 5.2 OpenYuma Source Files

이 섹션에서는 yuma-source 패키지에 포함된 파일에 대해 설명한다. 중요한 C 포함 파일은 **yuma-dev** 패키지가 설치될때 **/usr/include/yuma**디렉토리에 들어간다. **yuma-source** 패키지가 설치된 경우 전체 설치 소스 세트는 **/usr/share/yuma/src** 디렉토리에 들어간다. OpenYuma 도구는 이 기본 설치 위치를 확인하기 전에 **$YUMA_HOME/src** 하위 트리를 확인한다. (EX. 특정 임베디드 시스템에 대해 수정된 겨우 설치 사본 대신 OpenYuma 소스의 작업 사본을 사용할 수 있다.) 5.2장에서는 **netconf/src** 디렉토리에 포함된 파일이 나열된다.

### 5.2.1 src/ncx Directory

이 디렉토리에는 모든 OpenYuma 도구 프로그램에서 사용하는 **libncx.so** 바이너리 공유 라이브러리를 빌드하는데 사용되는 코드가 포함되어 있다. 모든 YANG / YIN, XML 및 XPath 처리를 포함하여 핵심 NETCONF/YANG 데이터 구조 지원을 처리한다.

표 4. src/ncx C Modules 

| C Module    | Description                                                  |
| :---------- | :----------------------------------------------------------- |
| b64         | YANG 바이너리 데이터 유형 인코딩 및 디코딩                   |
| blob        | SQL Blob 데이터 유형 인코딩 및 디코딩                        |
| bobhash     | bob 해쉬 기능 구현                                           |
| cap         | NETCONF 기능 정의 및 지원 기능                               |
| cfg         | NETCONF 데이터베이스 데이터 구조 및 구성 잠금 지원.          |
| cli         | YANG 정의에 의해 구동되는 CLI 매개 변수 구문 분석 데이터.    |
| conf        | 텍스트 .conf 파일 인코딩 및 디코딩, YANG 정의에 따른 데이터. |
| def_reg     | 일부 데이터 구조의 빠른 조회 지원을 위한 해쉬 기반 정의 레지스트리. 실제 데이터에 대한 백 포인터를 포함함 |
| dlq         | Double linked queue 지원                                     |
| ext         | YANG 확장 데이터 구조 지원                                   |
| grp         | YANG 그룹화 데이터 구조 지원                                 |
| help        | YANG 정의에 따른 데이터 기반의 자동 도움말 텍스트            |
| log         | System logging 지원                                          |
| ncx_appinfo | OpenYuma Netconf Extensions (NCX)  지원                      |
| ncx         | YANG 모듈 데이터 구조 지원 및 일부 유틸리티                  |
| ncx_feature | YANG 기능 및 if-feature 문 데이터 구조 지원                  |
| ncx_list    | YANG 비트 및 **ncx : xsdlist** 데이터 유형에 사용되는 **ncx_list_t** 데이터 구조 지원. |
| ncxmod      | 파일 관리 : YANG / YIN 파일, 데이터 파일 및 스크립트 파일 찾기 및 검색 제어 |
| ncx_num     | OpenYuma **ncx_num_t** 데이터 구조 지원. 값 노드 및 XPath 번호 처리에 사용됨. |

| C Module   | Description                                                  |
| ---------- | ------------------------------------------------------------ |
| ncx_str    | OpenYuma 문자열 지원.                                        |
| obj        | OpenYuma 개체 **(obj_template_t)** 데이터 구조 액세스        |
| obj_help   | 도움말 모듈과 함께 사용되는 자동화 된 개체 도움말 지원       |
| op         | NETCONF 작업 정의 및 지원 기능                               |
| rpc        | NETCONF \<rpc\> 및 \<rpc-reply\> 데이터 구조 및 지원 기능    |
| rpc_err    | NETCONF \<rpc-error\> 데이터 구조 및 지원 기능.              |
| runstack   | yangcli 스크립트에 대한 스크립트 실행 스택 지원              |
| send_buff  | NETCONF 송신 버퍼 기능                                       |
| ses        | NETCONF 세션 데이터 구조 및 세션 액세스 기능                 |
| ses_msg    | NETCONF 세션에 대한 메시지 버퍼링 지원                       |
| status     | 오류 코드 정의 및 오류 지원 기능                             |
| tk         | YANG, XPath 및 기타 구문을 구문 분석하는 데 사용되는 토큰 체인 데이터 구조. |
| top        | 최상위 XML 노드 등록 지원. \<rpc\> 및 \<hello\> 요소는 서버에 의해 등록됨. \<hello\>, \<rpc-reply\> 및 \<notification\> 요소는 클라이언트에 의해 등록됨. |
| tstamp     | 시간 및 날짜 스탬프 지원 기능                                |
| typ        | YANG typedef 데이터 구조 및 액세스 기능                      |
| val        | OpenYuma 값 트리 데이터 구조 및 액세스 기능                  |
| val_util   | 값 트리와 관련된 몇 가지 일반적인 SIL 작업을위한 고급 유틸리티. |
| var        | **yangcli** 및 (TBD) XPath에서 사용하는 사용자 변수 지원     |
| xml_msg    | XML 메시지 데이터 구조 및 지원 기능                          |
| xmlns      | XML 네임 스페이스 레지스트리                                 |
| xml_util   | XML 구문 분석 및 유틸리티 함수                               |
| xml_val    | XML 지원 **val_value_t** 데이터 구조 구성을위한 고급 지원 기능 |
| xml_wr     | XML 출력 지원 기능 및 액세스 제어 보호 메시지 생성 지원      |
| xpath1     | XPath 1.0 구현                                               |
| xpath      | XPath 데이터 구조 및 지원 기능                               |
| xpath_wr   | XML 인스턴스 문서 내에서 XPath 표현식 컨텐츠 생성 지원       |
| xpath_yang | 경로 표현식 및 인스턴스 식별자와 같은 특수 YANG XPath 구성 지원 |
| yang       | YANG 정의 및 일반 지원 기능                                  |
| yang_ext   | 확장 문의 YANG 구문 분석 및 유효성 검사                      |

| C Module   | Description                                                  |
| ---------- | ------------------------------------------------------------ |
| yang_grp   | 그룹화 문의 YANG 구문 분석 및 유효성 검사                    |
| yang_obj   | RPC, 알림 및 데이터 정의 명령문의 YANG 구문 분석 및 유효성 검사 |
| yang_parse | 최상위 수준의 YANG 구문 분석 및 유효성 검사 지원             |
| yang_typ   | YANG typedef 및 유형 문 지원                                 |
| yin        | YANG-YIN 매핑 정의                                           |
| yinyang    | YIN에서 YANG으로 번역                                        |

### 5.2.2 src/platfoem Directory

이 디렉토리에는 플랫폼 지원 포함 파일 및 Makefile 지원 파일이 포함되어 있다. 모든 OpenYuma C모듈에서 OpenYuma 프로그램과 사용되는 하드웨어 플랫폼 사이에 절연 레이어를 제공하는데 사용된다. 예를 들어 **m\_\_getMem**, **m\_\_getObj** 및 **m\_\_freeMem** 매크로는 malloc  및 free 함수 대신 직접 사용된다. 

표 5. src/platform Files

| File                    | Description                                                  |
| ----------------------- | ------------------------------------------------------------ |
| curversion.h            | SVNVERSION 번호를 얻기 위해 빌드 프로세스 중에 파일 생성     |
| platform.profile        | 빌드 지원을 위해 Makefile에 포함됨                           |
| platform.profile.cmn    | 빌드 지원을 위해 Makefile에 포함됨                           |
| platform.profile.depend | 종속성 생성 지원을 위해 Makefile에 포함됨                    |
| procdefs.h              | 플랫폼 정의. OpenYuma 코드 전체에서 사용되는 기본 데이터 유형 및 매크로를 포함. 모든 C 파일은 다른 OpenYuma 파일보다 먼저 이 파일을 포함합니다. |
| setversion.h            | curversion.h 파일을 생성하는 쉘 script                       |

### 5.2.3 src/agt Directory

이 디렉토리에는 NETCONF 서버 구현 및 내장 모듈 SIL 코드가 포함되어 있다. libagt.a 라는 정적 라이브러리가 빌드되고 netconfd 프로그램 내에서 정적으로 연결된다.

표 6. src/agt C Modules

| C Module      | Description                                                  |
| ------------- | ------------------------------------------------------------ |
| agt_acm       | NETCONF 액세스 제어 구현. **yuma-nacm** 모듈 SIL 콜백 함수를 포함함. |
| agt           | 서버 초기화 및 정리 제어 지점. **agt_profile_t** 데이터 구조도 포함 |
| agt_cap       | 서버 기능. server\\<capabilities \> 요소 컨텐츠를 생성       |
| agt_cb        | SIL 콜백 지원 기능.                                          |
| agt_cli       | 서버 CLI 및 .conf 파일 제어 기능.                            |
| agt_connect   | **netconf-subsystem**에서 **netconfd** 서버로 전송 된 내부 \\<ncx-connect \> 메시지를 처리함 |
| agt_hello     | 수신 클라이언트 \\<hello \> 메시지를 처리하고 서버 \\<hello \> 메시지를 생성함 |
| agt_if        | OpenYuma 인터페이스 모듈 구현. **yuma-interfaces** 모듈 SIL 콜백 함수를 포함함 |
| agt_ncx       | NETCONF 프로토콜 운영 구현. **yuma-netconf** 모듈 SIL 콜백 함수를 포함함 |
| agt_ncxserver | ncxserver 루프를 구현하여 서버 NETCONF 세션과 **netconf-subsystem** 씬 클라이언트 프로그램 간의 IO를 처리 |
| agt_not       | NETCONF 알림 구현. **notifications** 및 **nc-notifications** 모듈 SIL 콜백 함수를 포함 |
| agt_proc      | **/proc** 시스템 모니터링 구현. **yuma-proc** 모듈 SIL 콜백 함수를 포함 |
| agt_rpc       | NETCONF RPC 작업 처리기                                      |
| agt_rpcerr    | NETCONF \<rpc-error\> 생성                                   |
| agt_ses       | NETCONF 세션 지원 및 OpenYuma 세션 확장 구현. **yuma-mysession** 모듈 SIL 콜백 함수를 포함 |
| agt_signal    | 서버 신호 처리 지원                                          |
| agt_state     | 표준 NETCONF 모니터링 구현. **ietf-netconf-monitoring** SIL 콜백 함수를 포함 |
| agt_sys       | 서버 시스템 모니터링 및 알림 생성. **yuma-system** 모듈 SIL 콜백 함수를 포함 |
| agt_timer     | SIL 주기적 타이머 콜백 지원 기능                             |
| agt_top       | 서버 등록 및 최상위 XML 메시지 발송                          |
| agt_tree      | 하위 트리 필터링 구현                                        |
| agt_util      | SIL 콜백 유틸리티                                            |

| C Module      | Description                                                  |
| ------------- | ------------------------------------------------------------ |
| agt_val       | NETCONF 데이터베이스 작업에 대한 서버 유효성 검사, 커밋 및 롤백 지원 |
| agt_val_parse | 들어오는 \<rpc\> 및 \<config\> 콘텐츠 구문 분석 및 YANG 제약 조건 유효성 검사 완료 |
| agt_xml       | **ncx/xml_util** 함수에 대한 서버 XML 처리 인터페이스        |
| agt_xpath     | XPath filtering 구현                                         |

### 5.2.4 src/mgr Directory

이 모듈에는 NETCONF 클라이언트 지원 코드가 포함되어 있다. 모든 기본 NETCONF 세부 사항을 처리하므로 간단한 내부 API를 yangcli와 같은 NETCONF 애플리케이션에서 사용할 수 있다. libmgr.a 라는 정적 라이브러리가 yangcli 프로그램 내에 빌드되고 정적으로 연결된다.

표 7. src/mgr C Modules

| C Module      | Description                                                  |
| ------------- | ------------------------------------------------------------ |
| mgr           | 클라이언트 초기화 및 정리 제어 지점. 관리자 세션 제어 블록 데이터 구조 지원 기능도 포함 |
| mgr_cap       | 클라이언트 NETCONF \\<capabilities\> 요소 컨텐츠 생성        |
| mgr_hello     | 들어오는 서버 \<hello\> 메시지를 처리하고 클라이언트 \<hello\> 메시지를 생성 |
| mgr_io        | 클라이언트 NETCONF 세션에 대한 SSH 서버 IO 지원을 처리       |
| mgr_not       | 들어오는 서버 \<notification\> 메시지 처리                   |
| mgr_rpc       | NETCONF 서버로가는 \<rpc\> 메시지를 생성하고 NETCONF 서버에서 들어오는 \<rpc-reply\> 메시지를 처리 |
| mgr_ses       | 클라이언트 NETCONF 세션의 모든 측면을 처리합니다.            |
| mgr_signal    | 클라이언트 신호 처리기                                       |
| mgr_top       | 클라이언트 등록 및 최상위 XML 메시지 발송                    |
| mgr_val_parse | 들어오는 \<rpc-reply\>, \<notification\> 및 \<config\> 콘텐츠를 구문 분석하고 YANG 제약 조건 유효성 검사를 완료 |
| mgr_xml       | **ncx/xml_util** 함수에 대한 클라이언트 XML 처리 인터페이스  |

### 5.2.5 src/subsys Directory

이 디렉토리에는 netconf-subsystem이라는 하나의 C 소스 모듈을 포함한다. **netconf-subsystem**은 SSH 서버와 NETCONF 서버 사이에서 입력과 출력을 전송하는 thin-client 애플리케이션이며, 이는 **yuma-server** 패키지의 일부인 독립 실행형 바이너리이다. (**/usr/sbin** 디렉토리에 설치됨.)

### 5.2.6 src/netconfd Directory

이 디렉토리에는 NETCONF 서버 '주'기능을 정의하는 **netconfd**라는 C 모듈이 하나 포함되어 있다. **netconfd**는 **yuma-server** 패키지의 일부인 독립 실행형 바이너리이다. (**/usr/sbin** 디렉토리에 설치됨.)

### 5.2.7 src/yangcli Directory

이 디렉토리에는 OpenYuma NETCONF 클라이언트 프로그램인 yangcli 프로그램이 포함되어 있다. OpenYuma는 yuma-client 패키지의 일부인 독립 실행 형 바이너리이다. (**/usr/bin** 디렉토리에 설치됨.)

표 8. src/yangcli Directory

| C Module         | Description                                                  |
| ---------------- | ------------------------------------------------------------ |
| yangcli          | NETCONF 클라이언트 프로그램은 YANG 모듈을 기반으로 대화형 및 스크립트 기반 CLI를 제공 |
| yangcli_autoload | \<hello\> 메시지의 서버 기능을 사용하여 서버에서 누락 된 YANG 모듈을 자동으로 로드하고 모든 기능과 편차를 적용 |
| yang_autolock    | 상위 레벨 get-locks 및 release-locks 명령에 대한 프로토콜 교환 지원을 제공 |
| yangcli_cmd      | 주요 로컬 명령 프로세서                                      |
| yangcli_list     | yangcli 'list'명령 구현                                      |
| yangcli_save     | yangcli 'save'명령 구현                                      |
| yangcli_show     | yangcli 'show'명령 구현                                      |
| yangcli_tab      | 상황에 맞는 탭 단어 완성 구현                                |
| yangcli_util     | 다른 yangcli C 모듈에서 사용하는 유틸리티                    |

### 5.2.8 src/yangdiff Directory

이 디렉토리에는 OpenYuma YANG 모듈 비교 프로그램인 yangdiff 프로그램이 포함되어 있다. yangdiff는 yuma-client 패키지의 일부인 독립 실행 형 바이너리이다.(**/usr/bin** 디렉토리에 설치됨.)

표 9. src/yangdiff

| C Module      | Description                                    |
| ------------- | ---------------------------------------------- |
| yangdiff      | YANG 모듈 의미 비교 프로그램                   |
| yangdiff_grp  | YANG 그룹화 구문에 대한 의미 차이 구현         |
| yangdiff_obj  | YANG 데이터 정의 명령문에 대한 의미 차이 구현  |
| yangdiff_typ  | YANG typedef 및 유형 문에 대한 의미 차이 구현  |
| yangdiff_util | 다른 **yangdiff** C 모듈에서 사용하는 유틸리티 |

### 5.2.9 src/yangdump Directory

이 디렉토리에는 OpenYuma YANG 컴파일러 프로그램인 yangdump 프로그램이 포함되어 있다. yangdump는 yuma-client 패키지의 일부인 독립 실행형 바이너리이다. (**/usr/bin** 디렉토리에 설치됨.)

표 10. src/yangdump C Modules

| C Module      | Description                                                  |
| ------------- | ------------------------------------------------------------ |
| c             | SIL C 파일 생성 구현                                         |
| c_util        | SIL 코드 생성에 사용되는 유틸리티                            |
| h             | SIL H 파일 생성 구현                                         |
| html          | YANG to HTML 번역 구현                                       |
| sql           | YANG 모듈 웹 문서에 대한 SQL 생성 구현                       |
| xsd           | YANG에서 XSD 로의 번역 구현                                  |
| xsd_typ       | XSD simpleType 및 complexType 문에 YANG typedef/type 문을 구현합니다. |
| xsd_yang      | YANG에서 XSD 로의 번역 유틸리티                              |
| yangdump      | YANG 모듈 컴파일러                                           |
| yangdump_util | 모든 yangdump C 모듈에서 사용하는 유틸리티                   |
| yangyin       | YANG to YIN 번역 구현                                        |
...
- **ncxserver loop:**
  ...
  SSH2 서버는 'netconf' subsystem의 접속 연결 요청을 수신한다.<br/>새 세션 요청이 수신되면 netconf-subsystem 프로그램이 호출되어 netconfd 서버에 대한 로컬 연결을 연다.(ncxserver loop을 통해)<br/>NETCONF \<rpc\> 요청은 내부 NETCONF 스택에 의해 처리된다. 모듈 별 콜백 함수(파란색 상자)는 빌드 타임 또는 런타임 시스템에 로드할 수 있다. 이것은 SIL(서버 구현 라이브러리)이라고 하는 장치 계측 코드이다. 예를 들어 libtoaster의 경우 이것은 toaster 하드웨어를 제어하는 코드이다. 

- **Cleanup:**

  \<shutdown\> 또는 \<reboot\> 작업이 호출되면 서버가 정리됨. 재부팅의 경우 프로그램을 종료하는 대신 초기화 주기가 다시 시작됨

### 5.3.1 YANG Native Operation
...
## 5.4 Programs and Libraries Needed

### 5.4.1 SIL Shared Libraries

SIL 콜백 함수는 동적으로 로드되고 **dlopen** 및 **dlsym** 라이브러리 함수를 사용하여 서버에서 액세스 하는 공유 라이브러리로 빌드된다. 사용자 콜백함수와 해당 소스 파일은 "u\_" 접두사를 사용.  Yuma 콜백 함수 및 해당 파일은 "y\_" 접두사를 사용한다
...
**make_sil_dir** 스크립트는 단일 YANG 모듈에 대한 .C 및 .H 파일 코드 stub을 생성하는데 사용할 수 있다. 이 스크립트는 **yangdump-sdk** 프로그램을 사용하여 이러한 파일을 생성한다. 이 모드에서 **SIL 코드는 대상 모듈의 정의에 대해서만 생성**된다. 대상 모듈 (또는 다른 모듈)의 외부 기능 보강 문은 SIL 코드 생성에 영향을 주지 않는다.

단일 모듈 용 SIL 라이브러리는 **--module** 매개 변수를 사용하여 서버에 로드된다.

YANG 서브 모듈에 대해서는 SIL 라이브러리를 생성할 수 없다. **yangdump-sdk**를 사용하여 SIL 파일을 생성하는 경우 **--undefined = true** 옵션을 사용하여 전체 모듈에 대한 SIL 코드를 생성한다.

### 5.4.2 SIL Library Names
...
### 5.4.3 SIL Library Location

기본적으로 SIL 바이너리는 **/usr/lib/yuma** 디렉토리에 설치된다.

※ YUMA_RUNPATH는 SIL 라이브러리 바이너리 파일의 대체 위치를 지정하는데 사용된다.
...
### 5.4.5 Basic YANG Module Structure
...
### 5.4.6 Identifiers in YANG Modules
...
Zebos에 NETCONF 프로그램을 올리기전에 모듈이 정상적으로 load가 되고 \<get\>, \<get-config\>... 등의 작업이 정상적으로 동작하는지 확인할 필요성이 있다. 그렇기 때문에 OpenYuma package를 가상머신에 install 하여 확인해보는 작업을 거쳤다.

작업환경 : Ubuntu 18.04 LTS 환경에서 테스팅함<br>가상환경 만들기 위한 Virtual Box 설치(os 맞게 설치) : https://www.virtualbox.org/wiki/Downloads <br>우분투 18.04.5 LTS 64-bit PC(AMD64) desktop image 다운로드 링크 : https://releases.ubuntu.com/18.04.5/

oracle virtual box에 다운로드 받은 ubuntu install 과정 : https://makingrobot.tistory.com/19

![image](https://user-images.githubusercontent.com/50439622/102184560-fe09e080-3ef2-11eb-89c5-606bc4a8e307.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 16. 어댑터2에 호스트 전용 어댑터 설정

네트워크 -> 어댑터2 -> 호스트 전용 어댑터로 설정 (server-client 환경 구축하기 위해 local host가 아닌 ip 할당 받기 위함. 같은 ubuntu 환경의 client간 통신과 host와의 통신이 가능)<br>**※2021. 01.27 추가**<br>SG24장비와 통신하기 위해서는 네트워크 -> 어댑터2 ->  "어댑터에 브리지"로 설정 (Virtualbox가 호스트에 있는 네트워크 디바이스를 직접적으로 사용하는 네트워크. 호스트가 사용하는 네트워크 환경을 그대로 사용할 수 있으며, VM이 호스트와 같은 물리적인 네트워크에 존재하게 할 수 있다.)<br>Ubuntu 고정 ip 설정 방법 : https://www.manualfactory.net/10108

![image](https://user-images.githubusercontent.com/50439622/102454303-258eb380-4081-11eb-8a46-771f53807204.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 17. ifconfig를 통해 확인한 ip(호스트 전용 어댑터)

진행하기에 앞서..

OpenYuma installtion quide : https://github.com/OpenClovis/OpenYuma/blob/master/netconf/doc/yuma_docs/openyuma-installation-guide.odt<br>YumaPro installation guide : https://www.yumaworks.com/pub/docs/latest/pdf/yumapro-installation-guide.pdf

처음 OpenYuma installation guide로 install을 진행했을때, 특정OpenYuma Package의 .deb 파일들의 install이 제대로 되지 않는 문제점이 있었다. 관련 doc 을 찾아보아도 해결방안을 찾기 힘들어 OpenYuma대신 YumaPro에서 진행했던 방식대로 Server, Client 관련 Library들을 다운로드 받았으며, https://github.com/OpenClovis/OpenYuma 에서 Open source 코드 다운로드 받은 후 make / sudo make install 진행 했다. 

OpenYuma와 YumaPro의 차이점을 찾아보았을 때, Openyuma에서 없는 기능들이 상당부분 YumaPro에는 존재했음을 확인할 수 있었다. (ex: make_sil_bundle command 등) Openyuma 관련 작업은 대부분 5-8년전 작업을 한것에 비해 YumaPro SDK 버전은 계속해서 작업이 진행되는 것에서 오는 차이인것 같다.

## 6.1 Server / Client 관련 lib 설치

### 6.1.1 External Package Needed by the Server

```shell
mydir> sudo apt-get install libxml2-dev
```

일부 XML 구문 분석 기능에 필요. Linux 내에 default make로 설치되어 있지만, openyuma를 빌드하려면 DEB 기반의 libxml2-dev라는 개발자 패키지가 필요함.
...
### 6.1.2 External Packages Needed by the Client
...
## 6.2 OpenYuma Opensource Download

https://github.com/OpenClovis/OpenYuma

![image](https://user-images.githubusercontent.com/50439622/102184906-9011e900-3ef3-11eb-90e5-deb7b725d98e.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 18. OpenYuma OpenSource 화면

초록박스의 **Code**-> **Download ZIP** 클릭하여 파일 다운로드한다. (git clone으로 받는 방법이 더 빠름)

![image](https://user-images.githubusercontent.com/50439622/102185336-4249b080-3ef4-11eb-8fd1-819ab7db8194.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 그림 19. download 받은 .zip 파일

다운로드 받은 OpenYuma-master.zip 파일 압축을 해제한다.

![image](https://user-images.githubusercontent.com/50439622/102185582-981e5880-3ef4-11eb-8a88-1ac2aa0905e9.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 20. unzip 작업 수행

OpenYuma-master파일 $HOME (/home/dylee) 경로로 복사한다.

※ [5.1.1 Searching OpenYuma Roots](#511-searching-openyuma-roots) 참고해보면 $HOME 디렉토리가 첫 검색 루트이기 때문

![image](https://user-images.githubusercontent.com/50439622/102185745-e0d61180-3ef4-11eb-876e-0edcbfeb234b.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<그림 21. OpenYuma-master $HOME 디렉토리로 복사
...
이 파일에 OpenYuma 구성 매개변수에 대한 모든 기본 설정이 포함되어 있다.

netconfd-sample.conf 파일은 **-sample**이 붙은 sample 파일이기 때문에 이를 netcofd.conf 파일로 복사하여 기본 구성 매개변수 파일 생성할 필요가 있다.<br/>수정할 경우는 편집기 사용하여 수정 (아래 예시에서는 emacs 사용)
...
NETCONF 서버를 시작하려면 sshd 서버가 실행 중인지 확인하고 구성이 **/etc/ssh/sshd_config**에 포함되어 있는지를 확인해야 한다. **/etc/ssh/sshd_config**에 아래 3줄 추가(Port 22 는 주석처리 되어 있음)
...
'Port 22' 명령은 SSH 서버가  NETCONF 세션 외에도 SSH 세션을 허용하는지 확인하는데 필요하다.

- **Installing SSH keys (필수 X)**
...
OpenYuma_master 디렉토리에 README 파일 참고하여 compile 수행한다.<br>
...
netconfd server 실행하기 앞서 OpenYuma Package 위치 확인이 먼저 필요하다.

```
1. /usr/bin 디렉토리
  -> yangcli (server 필수는 아님)
  -> yangdiff
  -> yangdump
2. /usr/sbin 디렉토리
  -> netconfd
  -> netconf-subsystem
3. /usr/lib 디렉토리
  -> libncx.so.2.1 (파일 확장자는 플랫폼에 따라 다를 수 있음)
  -> libagt.so.2.1 (파일 확장자는 플랫폼에 따라 다를 수 있음)
4. /usr/share/yuma/modules 디렉토리
  ->
5. /etc/yuma 디렉토리
  -> yangcli-sample.conf
...
원하는 방식으로 수행하도록 서버를 조정하는데 사용할 수 있는 많은 매개변수가 있다.
--log-level : 가장 자세한 디버그 메시지 수준으로 서버 시작(debug4 : 최상, debug : 최하)
--access-control : 서버가 수행하는 작업을 모니터링하고 액세스 제어를 비활성화(NETCONF 액세스 제어를 설정하지 않고도 YANG 데이터 조작 가능)
...
NETCONF 서버가 실행되면 클라이언트 세션을 수락하게 된다..
...
- To start the client interactively, and send all log messages to STDOUT:
  ...
  ![image](https://user-images.githubusercontent.com/50439622/102188593-d6b61200-3ef8-11eb-8079-fcaa63053c5b.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 그림 22. install yangcli

  이러한 오류 메시지 발생 시 **sudo apt install yangcli** 수행 해준다.
...
**'connect'** 명령은 NETCONF 세션을 시작하는데 사용되며, 3개의 필수 parameter가 있다.
...
서버 기능 및 기타 정보를 자세히 설명하는 서버의 'hello' 메시지가 표시되며, 서버가 실행중인 터미널 창에서 서버가 시작된 디버그 수준에서 표시되는 디버그 정보를 볼 수 있다.(debug level은 1-4까지 존재. debug ~ debug4)

이제 다음과 같은 일부 명령을 실행하여 YANG 데이터를 표시할 수 있다. <br>sget, xget의 차이점 참고 : [4.1 Data Retrieval](#41-data-retrieval)

![image](https://user-images.githubusercontent.com/50439622/102189243-b20e6a00-3ef9-11eb-8964-50355678c393.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 24. get operation을 통한 session 정보 가져옴

yangcli 및 사용 가능한 명령에 대한 자세한 내용 : https://github.com/OpenClovis/OpenYuma/blob/master/netconf/doc/yuma_docs/openyuma-yangcli-manual.odt

yangcli 종료하려면 'quit' 명령어 사용

```
dylee@192.168.56.104> quit
```

NETCONF seesion을 종료하려면

```
dylee@192.168.56.104> close-session
```

NETCONF 서버를 종료하려면 < Ctrl > + c

# 7. OpenYuma 생성된 bin 파일, 기본 yang 파일 path 및 size

Zebos에 NETCONF 프로그램을 올리기 전에 관련 bin 파일, yang 파일의 용량및 디렉토리 구조를 파악할 필요성이 있다. 7장에서는 그에 대한 관련 bin 파일, yang 파일에 대한 용량 및 디렉토리 구조를 확인해보았다.

## 7.1 .yang 파일 경로 및 각 디렉토리 용량

```
> /usr/share/yuma/modules/<ietf,netconfcentral,test,yang>
```

![image](https://user-images.githubusercontent.com/50439622/102463649-b8821a80-408e-11eb-8f09-109e2fb68c53.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 그림 25. yang 파일 전체 용량

## 7.2 netconfd 파일 경로 및 용량

```
> /usr/sbin/netconfd
```

![image](https://user-images.githubusercontent.com/50439622/102464032-33e3cc00-408f-11eb-9f12-bd289473c2cc.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 26. netconfd 파일 용량

## 7.3 yangcli 파일 경로 및 용량

```
> /usr/bin/yangcli
```

![image](https://user-images.githubusercontent.com/50439622/102464588-dd2ac200-408f-11eb-96a8-3819ffdc0047.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 그림 26. yangcli 파일 용량

# 8. OpenYuma \<Toaster Module\> load test

OpenYuma에서는 Toaster라는 모듈을 기본 테스트용으로 제공하고 있다. 이를 통해 Install 과정 및 configuration 과정이 제대로 이루어졌는지 테스트해볼 필요성이 있다. 8장에서는 Toaster Module을 Load 해보고 \<get\>, \<notification\> 과정을 테스트 해보았다.

## 8.1 Enable Notification Delivery

기본 NETCONF 알림 스트림은 **'create-subscription'**명령으로 시작할 수 있다.

```shell
dylee@192.168.56.104> create-subscription
```

NETCONF 서버내의 다른 활동에 따라 'sysSessionStart' 또는 'sysSessionEnd'와 같은 다른 알림 이벤트가 생성 될 수 있다. 알림은 전체적으로 표시되지만 'rpc reply ouput' 중에는 출력되지 않는다.

## 8.2 Load the Toaster Module

Toaster 모듈은 핵심 시스템 모듈이 아니며 자동으로 사용할 수 없다.
모듈은 NETCONF 클라이언트에 의해 명시적으로 로드되어야 한다.

```shell
dylee@192.168.56.104> load toaster
```

모듈이 성공적으로 로드되면 로드된 토스터 모듈의 개정 날짜가 포함된 데이터 응답이 전송됨.
이 응답은 이미 로드된 경우에도 반환된다.

![image](https://user-images.githubusercontent.com/50439622/102460618-9b4b4d00-408a-11eb-83b0-29ffb38dd291.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 그림 25. 정상 load toaster 되었을때의 rpc-reply

'sysCapabilityChange' 알림 이벤트는 모듈이 서버에 아직 로드되지 않은 경우에만 전송됨.
이 경우 이 세션에 대한 \<hello\> 메시지에 광고되지 않았으므로 Toaster 모듈을 로드해야 한다.

```shell
dylee@192.168.56.104> mgrload toaster
```

![image](https://user-images.githubusercontent.com/50439622/102558469-d3956e80-4110-11eb-8678-344e72188ef1.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 그림 26. mgrload toaster return

## 8.3 Enable the Toaster

```
dylee@192.168.56.104> make-toast (error 메시지 출력될 것임)
...
- **Lock the Databases**
  ...
  ```shell
  dylee@192.168.56.104> get-locks
  ```

- **Create the toaster Container**

  toaster 모듈은 간단한 YANG 'presence' 컨테이너를 사용하여 toaster 서비스를 구성./toaster 컨테이너가 생성되면 컨테이너 내의 읽기 전용 노드가 서버에 의해 유지되고 toaster 서비스가 활성화 됨.
  첫 단계는 \<candidate\> config 데이터베이스에 /toaster 노드를 만드는 것.
  
```shell
  dylee@192.168.56.104> create /toaster
```

- **Save the Database Changes**

  현재 세션의 데이터베이스 대상에 따라 **'commit'** 또는 **'copy-config'** 작업을 실행하는 high-level yangcli 작업

  ```shell
  dylee@192.168.56.104> save
  ```

- **Unlock the Databases**

  ```
  dylee@192.168.56.104> release-locks
  ```

$HOME/.yuma 디렉토리의 startup-cfg.xml 파일을 열어보면 아래의 그림27과 같이 toaster module이 적재된것을 확인할 수 있다.

![image](https://user-images.githubusercontent.com/50439622/102461406-ab176100-408b-11eb-9b70-d3f558938217.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 그림 27.  startup-cfg.xml 파일

## 8.4 Get the Toaster State Information (GET Test)

토스터 모델과 현재 상태를 발견하기 위해 'sget' 또는 'xget' 명령을 사용하여 서버에서 사용 가능한 개념적 상태 데이터의 토스터 부분만 검색 할 수있다.<br>
'sget' 명령은 **\<get\>** 작업에 대한 상위 수준의 **subtree filter** 처리기.
...
## 8.5 Start/Stop Making Toast (notification Test)
...
# 9. IETF 표준 Module load test

이 장에서는 IETF 실제 표준 Module을 통해 Client에서 Remote하고 Server에서 \<get\>, \<get-config\>, \<edit-config\> 등의 작업을 정상적으로 수행하는지 확인하려고 한다.

## 9.1 OpenYuma 내에서 사용할 **서버계측**을 만드는데 필요한 단계

서버계측은 SIL([5.1.2  What is a SIL ?](#512-what-is-a-sil-))에서 수행된다.

- YANG 모듈 데이터 모델 정의를 **생성**하거나 **기존 YANG 모듈을 사용**한다. (개발 시간이 많지 않기 때문에 우선적으로 기존 YANG 모듈 사용에 중점)<br>※ **yangdump 프로그램** : YANG 모듈의 유효성을 검사하고 오류가 없는지 확인 한다. 또한 모든 경고를 조사하여 데이터 모델링 버그를 나타내는지 여부를 확인해야 한다.(SIL이 수행되면 yangdump도 함께 수행된다고 생각하면 될 것같다.)
- **$YUMA_HOME**[5.1.1 Searching OpenYuma Roots](#511-searching-openyuma-roots) 환경변수가 정의되어 있고 OpenYuma 개발 트리를 가리키는지 확인해야 한다.
- SIL 개발 하위 트리를 만들어야 한다.<br>**/usr/bin** 디렉토리에 설치된 **make_sil_dir** 스크립트를 사용하여 디렉토리 구조와 Makefile을 생성한다. 이 단계에서 **yangdump를 호출하여 SIL의 초기 .H, .C 파일을 생성**하게 된다.
- 텍스트 편지기(vi, emacs ...)를 사용하여 각 개체, RPC 방법 및 알림에 대한 장치 별 계측을 채운다. 거의 모든 가능한 NETCONF 관련 코드는 중앙 스택에서 처리되거나 자동으로 생성된다. 따라서 이 코드는 YANG 데이터 모델의 의미를 구현한다.
- **Compile code**<br>SIL **'src' 디렉토리**에서 **'make'** 명령을 사용한다. SIL **'lib' 디렉토리**에 라이브러리 파일이 생성되어야 한다.
- **netconfd 서버**에서 사용할 수 있도록 SIL 라이브러리를 설치한다.<br>SIL **'src'디렉토리**에서 **'make install' 명령**을 사용한다.
- **netconfd Server 실행**
- **새로운 Module Load**<br>재부팅 할 때마다 모듈을 로드해야하는 경우 구성 파일에 'load' 명령을 추가해야 한다. (yangcli에서 
  **'load <Module 명>'**)
- **netconfd** Server는 지정된 YANG 모듈과 SIL을 로드하여 모든 세션에서 사용할 수 있도록 한다.

## 9.2 기존 IETF 모듈을 통한 서버 계측 구성

### 9.2.1 각 파일 위치 및 용량 확인

![image](https://user-images.githubusercontent.com/50439622/102562395-54a53380-411a-11eb-8ec9-787c4419b9d0.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 28. make_sil_dir 디렉토리 경로 / ietf yang 파일 경로

### 9.2.2 make_sil_dir 수행을 통해 디렉토리 구조와 Makefile을 생성

ietf 모듈의 ietf-interfaces yang파일을 가지고 test

![image](https://user-images.githubusercontent.com/50439622/102562754-1bb98e80-411b-11eb-82c4-5f08486afab5.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 29. make_sil_dir 수행

**$YUMA_HOME** 환경변수로 설정되어 있는 /OpenYuma-master/netconf 경로에서 **make_sil_dir 명령어**를 수행해주었다 ietf-interfaces라는 이름의 파일명이 생성되는것을 확인할 수 있다. make_sil_dir은 **'combined' SIL files** 또는 **'split' SIL files**을 생성할 수 있다.

- foo.yang 파일에 대한 **'split' SIL 모듈**은 다음 파일을 사용하여 생성된다. 

  표4 foo.yang 파일에 대한 'split' SIL 모듈 이용한 파일 생성<br>

  | FILE NAME | TYPE         | Description                                                  |
  | --------- | ------------ | ------------------------------------------------------------ |
  | u_foo.c   | User SIL     | 'foo'모듈에 대한 사용자 제동 서버 계측 코드                  |
  | u_foo.h   | User SIL     | 'foo'모듈에 대한 사용자 제공 외부 정의 (**Should not edit**) |
  | y_foo.c   | OpenYuma SIL | 'foo' 모듈에 대한 OpenYuma 서버 glue code (**Do not edit**)  |
  | y_foo.h   | OpenYuma SIL | 'foo' 모듈에 대한 OpenYuma 서버 외부 정의 (**Do not edit**)  |

- foo.yang 파일에 대한 **'combine' SIL 모듈**은 다음 파일을 사용하여 생성된다.

  표5. foo.yang 파일에 대한 'combine' SIL 모듈 이용한 파일 생성<br>

  | FILE NAME | TYPE                        | Description                                                  |
  | --------- | --------------------------- | ------------------------------------------------------------ |
  | foo.c     | OpenYuma 및 사용자 SIL 결합 | 'foo' 모듈에 대한 사용자 제공 서버 계측 코드                 |
  | foo.h     | OpenYuma 및 사용자 SIL 결합 | 'foo' 모듈에 대한 사용자 제공 외부 정의 (**Should not edit**) |

### 9.2.3 compile code 및 SIL 라이브러리 설치

![image](https://user-images.githubusercontent.com/50439622/102566367-bff30380-4122-11eb-925f-680a337c3579.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 30. make 수행

**make_sil_dir** 수행을 통해 ietf-interfaces 디렉토리가 생성되었고 ietf-interfaces 디렉토리의 **/src** 경로에서 make를 수행해 code를 compile 해준다.

 ![image](https://user-images.githubusercontent.com/50439622/102573175-35fe6700-4131-11eb-9546-d5f6e860ca7d.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 31. make install 수행

code compile 수행 이후 **netconfd 서버**에서 사용할 수 있도록 **make install** 명령어 통해 SIL 라이브러리를 설치한다.

![image](https://user-images.githubusercontent.com/50439622/102566837-a605f080-4123-11eb-885f-a4228284d8f6.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 32. libietf-interfaces.so 파일 생성

**/usr/lib/yuma** 디렉토리 경로에 libietf-interfaces.so 파일이 생성된 것을 확인할 수 있다. 여기 libietf-interfaces.so 파일의 naming은 'lib' + <module 명> + '.' + 확장자명(so) 로 붙여지는 규칙이 있다. (ex : 'dylee'라는 이름의 yang module일 경우 'libdylee.so')

### 9.2.4 yangcli 를 통해 ietf-interfaces load

먼저 [링크](#66-yangcli-connect)에서와 동일하게 netconfd server와 yangcli client를 활성화 시켜주어야 한다.

![image](https://user-images.githubusercontent.com/50439622/102567488-f3369200-4124-11eb-8782-440a5155a383.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 33. ietf-interfaces load

**create-subscription** 명령어 이후 ietf-interfaces 를 load 시켜준다.

![image](https://user-images.githubusercontent.com/50439622/102573264-6cd47d00-4131-11eb-842c-b4911f0146d1.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 34. ietf-interfaces mgrload

이후 **mgrload** 명령어 수행한다.

![image](https://user-images.githubusercontent.com/50439622/102567701-6c35e980-4125-11eb-97f4-3fa753156e62.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 35. ietf_interfaces.yang

ietf_interfaces.yang 파일을 확인해 보면 **interfaces 라는 container node**가 존재한며 그 하위 **list node**로 **interface node**가 존재한다. **interface node**의 하위 **leaf node**에는 **"name", "description", "type", "enabled", "link-up-down-trap-enable"** 등이 존재한다.<br>※ ietf-interfaces.yang 파일 위치 : **/OpenYuma/netconf/module/ietf/** 에 위치

![image](https://user-images.githubusercontent.com/50439622/102568390-bcfa1200-4126-11eb-8b89-3b43169e79f9.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 36. create interfaces list node

ietf-interfaces 모듈을 load 했기 때문에 interfaces 서비스 활성화가 가능하다. **create /interfaces**를 통해 interfaces를 활성화 해준뒤 **save**를 통해 **"commit", "copy-config"** 해준다.<br>※ 작업 전 **get-locks**, 작업 후 **release-locks**을 통해 다른 session에서 수정하지 못하도록 해야함

![image](https://user-images.githubusercontent.com/50439622/102573343-a1e0cf80-4131-11eb-8a65-f8a9e81c9c7a.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 37. startup-cfg.xml 파일

$HOME/.yuma 디렉토리에 존재하는 startup-cfg.xml 파일을 열어보면 \<interfaces\>가 configuration 파일에 load 되었음을 확인 할 수 있다.



# 10. yang-explorer(cisco yang data model)

※ 2021.01.12 추가<br>Yang-explorer는 Adobe flash player 멀티미디어 애플리케이션을 사용하는데 Adobe flash player는 수많은 보안 취약점의 원인으로 현재 모든 브라우저에서 보안상의 문제로 Adobe flash player에 대한 서비스를 중지했다. yang-explorer 사용은 더이상 이용할 수 가 없을것 같다..

관련 docs : https://www.cisco.com/c/ko_kr/support/docs/storage-networking/management/200933-YANG-NETCONF-Configuration-Validation.html <br>관련 Opensource link : https://github.com/CiscoDevNet/yang-explorer<br>관련 tutorial 동영상 : https://www.youtube.com/watch?v=sPwxPGNtSaU

10장에서 작성된 내용은 "관련 Opensource link"의 README 파일을 참고해서 작성되었음.

## 10.1 Installation

### 10.1.1 Prerequisite(전제 조건)

- MAC, Linux (Windows에서는 지원 안됨)

- 파이썬 2.7 ===> 설치문서https://ndb796.tistory.com/343

- pip 패키지 관리자 ===> 설치문서(https://pip.pypa.io/en/stable/installing)

  파이썬 버전별 pip설치 ===> http://wanochoi.com/?p=3575
...
- python.h 또는 xmlversion.h 누락으로 인해 설치 오류가 발생하면 종속성 패키지 설치
Ubuntu: sudo apt-get install libxml2-dev libxslt1-dev python-dev zlib1g-dev
```

## 10.2 Running YangExplorer

### 10.2.1 Running with localhost
...
- **Start Server:**
  ...
  note 1 : 설치 중에 virtualenv를 사용하지 않은 경우 sudo가 필요할 수 있음
  note 2 : pyang not found.. please install pyang before continuing!! 오류 메시지 나올 경우
           https://github.com/CiscoDevNet/yang-explorer/wiki -> pyang install 참조
  ```
...
- **Start Explorer**
  ...
  ![image](https://user-images.githubusercontent.com/50439622/102747546-d0fe7780-43a3-11eb-8652-b4da1638af4d.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 38. yang-explorer localhost GUI

  서버를 실행(./start.sh)하게 되면 connection이 맺어지고, "Use http://localhost:8088/static/YangExplorer.html" 링크를 열게되면 그림 38과 같은 yang-explorer GUI가 열리게 된다.

  ※adobe flash player 설치가 되어 있지않으면 설치 후 진행 : https://brown.ezphp.net/entry/%EC%9A%B0%EB%B6%84%ED%88%AC-1804%EC%97%90-flash-player-%EC%84%A4%EC%B9%98%ED%95%98%EB%8A%94-%EB%B0%A9%EB%B2%95

### 10.2.2 Running with ip-address (Shared Server)
...
- **Start Server:**
  ...
  var flashvars = {}; 
  + flashvars.host = '<ip-address>'; -> 초기 local host로 잡혀있음
  + flashvars.port = '8088';
  ...
  cd <install-root>/yang-explorer
  vi start.sh
  (update HOST variable with <ip-address>) -> 초기 localhost로 잡혀있음
  # save & quit
        
  ./start.sh
  ```

  ![image](https://user-images.githubusercontent.com/50439622/102746169-0ce40d80-43a1-11eb-96df-7fe335e9753e.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 39. yang-explorer ifconfig

  현재 host ip address : 192.168.56.101<br>위에서 언급된 **YangExplorer.html**파일에 **flashvars.host**에 host ip address 설정, port 설정<br>**start.sh** host ip address 설정
...
~~가장 중요한것은 가상환경내에 **chrome을 설치**해야 한다.(firefox에서 yang model upload가 안됨.) server와 connection을 맺고 해당 server 링크 단순히 **<ctrl\>+click** 하게 되면 default로 firefox browser로 접속되게 됨(주의)~~<br>~~chrome 다운로드 : https://coding-factory.tistory.com/498~~

※2020.12.22 추가<br>2020년 까지를 기준으로 더이상 chrome에서 adobe flash player를 지원하지 않는다고 한다. firefox browser에서 localhost로 진행해도 무방할것 같다는 판단에(Cisco example에서도 localhost로 netconfd 서버 ssh 연결함) firefox browser이용 localhost에서 User Guide 진행. 

### 10.3.1 Work Flow(Login)

YangExplorer는 사용자 계정을 사용하여 작업 공간을 관리한다. 관리자 페이지를 사용하여 계정을 생성하거나 **기존 설정된 계정(guest/guest)**을 사용하여 로그인 할 수 있다.

![image](https://user-images.githubusercontent.com/50439622/102753925-7d922680-43af-11eb-84c4-cc359dfccf50.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 40. yang-explorer login

### 10.3.2 Work Flow(Adding / Deleting Yang Models)

![image](https://user-images.githubusercontent.com/50439622/102963064-91629780-452b-11eb-958e-c9484d8dfb9b.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 41. Add/Delete Yang Module

- **Manage Models** : User Profile내의 현재 Yang Models
- **Subscribed/Un-Subscribe** :  Explorer Area에서 보여지는 Model 표시 (현재 Subscribed Model은 ietf-interfaces 하나)
- **Add/Delete** : 새로운 Model 업로드 / 선택 된 Models 삭제

### 10.3.3 Work Flow(Yang Explorer 애플리케이션 사용)

![image](https://user-images.githubusercontent.com/50439622/102865606-5b64db00-4479-11eb-8c5d-0a9ca29ac8d0.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 42. yang-explorer client 접속

현재 netconfd(OpenYuma server)가 실행되고 있는 **server의 ip address는 192.168.56.104**이며, **client(yang-explorer)의 localhost(192.168.56.101)을 통해 접속을 시도**했다. **platform**란에 **other**를 선택하고, **Host(192.168.56.104)**, **Port(830)**, **ID/PW** 입력후 **Capabilities**를 클릭하게 되면 server에서 보낸 hello message를 받게된다.


- **데이터 검색**

  Server <-> Client 간 Session 연결이 되었기 때문에 ietf-interfaces yang 모듈의 데이터를 검색하여 가져올 수 있다.

  ![image](https://user-images.githubusercontent.com/50439622/102960899-51e57c80-4526-11eb-8157-973fb7bce1cb.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 43. yang-explorer data 검색

  **ietf-interfaces** yang 모듈의 **name**을 검색하기 위해 **Oper(Operation)**를 선택하고 그 뒤에 **interface**의 **name**항목의 **Values**란을 클릭하게 되면 드롭박스로 **\<get-config\>, \<get\>** 등의 Operation을 선택할 수 있다. **Operation** 선택 후 **Netconf** 설정의 **RPC**를 클릭하게 되면 해당하는 모듈의 데이터를 가져오게 된다.

  ![image](https://user-images.githubusercontent.com/50439622/102960983-91ac6400-4526-11eb-853e-bbdd7542cece.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 44. yang-explorer RPC Run

  YANG 형식 NETCONF RPC 메시지가 생성되면 **CustomerRPC** 설정 후 **Run**을 클릭하면 netconfd(OpentYuma) Server로 메시지를 전송하게 되고, netconfd(OpenYuma)는 메시지를 받고 메시지에 해당하는 데이터를 client(yang-explorer)에게 전달한다. (현재 data 노드는 공백이기 때문에 전달되는 메시지 없음.)

  

- **YANG 데이터 모델 파일 대량 로드**

  YANG 모듈 모음 포함된 저장소 : https://github.com/YangModels/yang

  위 링크의 저장소에서는 IETF 표준 추적 YANG 모듈, 공급업체별(Cisco, Ciena 등 ) YANG 모듈등 YANG 모듈을 모아놓은 저장소이다.

  모든 벤더에 적용되는 표준(IETF:Internet Engineering Task Force) 모델은 **standarad/ietf/rfc** 디렉토리에서 찾을 수 있다. 이는 IETF 표준 기관이 RFC 게시에 제공한 **표준 기반 YANG 데이터 모델**을 제공한다.

  IETF 표준 모듈 모음 저장소 : https://github.com/YangModels/yang/tree/master/standard/ietf/RFC
  ...
  cd server
  python manage.py --user <username> --git <git-url> --dir <path_to_yang_model>
  
  1) local에서 upload
  python manage.py bulkupload --user guest --dir <local_path_to_yang_model>
  
  2) git에서 upload
  python manage.py bulkupload --user guest --git https://github.com/YangModels/yang.git --dir standard/ietf/RFC
  
  ※ virtualenv 종료 명령어 : deactivate
  ```

  어떤 오류라도 발생 시 yang model은 제대로 업로드 되지 않는다.

  ```
note 1 : No module named django.core.management 에러 발생시
           Ubuntu > sudo pip install -U Django==1.8.3
  note 2 : No module named graphviz 에러 발생시
           Ubuntu > sudo pip install graphviz
  note 3 : pyang 관련 install 진행해야 할 경우
           관련 자료 : https://github.com/mbj4668/pyang 참조하여 pyang install 
  ```
  
   ![image](https://user-images.githubusercontent.com/50439622/103252281-7e364880-49bf-11eb-82d2-7d0dd856225b.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 45. cisco yang model bulkupload

  YANG 모듈 모음 저장소에서 cisco vendor의 모듈들을 대량 업로드 진행 해당 yang 모듈들을 찾아 upload를 진행한다.

  ![image](https://user-images.githubusercontent.com/50439622/103253198-7e384780-49c3-11eb-8c59-8359b59bffb5.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 46. cisco yang model upload

  browser 접속을 통해 정상적으로 cisco yang model이 upload 되었는지 확인 가능하다. 총 74개의 cisco yang model이 upload 된 것을 확인할 수 있다.

  모든 yang model 선택 후 Subscribe 적용하게 되면 Explorer 창에 모든 yang module을 visible 상태로 놓고 설정 할 수 있다.

  ![image](https://user-images.githubusercontent.com/50439622/103253824-0ddef580-49c6-11eb-97d0-b6fcf9756d2f.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 47. cisco yang model(cisco-ia save-config) RPC

  cisco-ia.yang 데이터 모델을 예제로 실행해 보았다. save-config 작업을 선택하고 RPC를 클릭하면 관련 RPC가 생성된다.

  ![image](https://user-images.githubusercontent.com/50439622/103253890-5c8c8f80-49c6-11eb-98e3-c12885ae1903.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 48. cisco yang model(cisco-ia save_config) Run

  NETCONF를 통해 server(OpenYuma)로 RPC 메시지를 전송하기 위해 Run 을 클릭한다. server에 관련 작업 모델이 존재하면 성공적으로 수행이 되지만, 현재 server에는 cisco 관련 yang model 데이터를 처리할 수 없기 때문에 \<rpc-error\> 메시지를 보낸다.

  **※ 위의 방식 대로 cisco vendor 관련 모듈이 아닌 ietf 표준 모듈을 업로드하려고 했지만 compile error가 남**

  > git clone https://github.com/YangModels/yang.git

  위 명령어 통해 원하는 디렉토리에 직접 모듈 모음 저장소 다운로드 받아서 이용
  
  ![image](https://user-images.githubusercontent.com/50439622/103260655-c23a4500-49e1-11eb-968a-97a7762d9c5b.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 49. ietf 표준 yang 모듈 upload

  **Manage Models -> Add -> Browse** 로 이동해 yang 모듈 모음 저장소를 받은 디렉토리로 들어간다. 이후 standard/ietf/RFC로 이동하면 ietf 관련 yang 모듈들이 있다. 모든 모듈들 선택한 후 Open을 누르면 status ready 상태로 Upload Yang에 적재된다.(upload 중 compile error가 발생하는 yang model도 있음. ietf yang model은 사용가능한 model들 선택하여 제한적 업로드 가능)
  
  

## 10.4 Toaster.yang(OpenYuma sample yang model)을 이용한 yang-explorer test

ietf 표준 모듈을 통해 test를 진행해보려 했지만, netconf server가 실제 장비에 물려있는 것이 아니기 때문에 해당 설정 정보를 제대로 가져오지 못했다. 수행은 정상적으로 되지만, (데이터값을 찾을 수 없기때문에 가져오지 못하는 현상) 그래서 yang-explorer에서 load한 모듈을 가지고 server에서 데이터값을 가지고 오는지 확인하기 위해 OpenYuma 에서 기본으로 제공하는 toaster.yang 파일을 가지고 test해보기로 하였다.(**결론적으로는 OpenYuma에서 변경된 stub 파일을 yang-explorer에서 제대로 받아오는지 확인 하기 위한 것이기 때문**)

- **Toaster.yang 모듈 load**

  ![image](https://user-images.githubusercontent.com/50439622/103340095-98a71980-4ac6-11eb-82cf-ebbfc5532b1c.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 50. toaster.yang model upload(1)

  yang-explorer(cisco application)를 통해 yang model 사용하기 위해 upload를 먼저 해주어야한다. toaster.yang 모델의 경우 기존 yang-explorer 기본 제공 model에는 존재하지 않기 때문에 **openyuma/netconf/module/netconfcentral** 에서 찾아 upload 해주어야 한다. (server <-> client 간에는 동일한 yang 파일이 존재해야함.)

  ![image](https://user-images.githubusercontent.com/50439622/103340703-16b7f000-4ac8-11eb-901e-d59c0b789615.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 51. toaster.yang model upload(2)

  toaster.yang 파일 업로드 이후 compiling 시간을 기다린 후에 위 그림과 같이 toaster 모듈이 workspace에 적재된 것을 확인할 수 있다. 이후 **하단 Subscribe를 클릭하게 되면 Explorer 화면에 toaster 모듈의 작업이 가능**해진다.

- **Toaster.yang 모듈 \<get\> test**

  ![image](https://user-images.githubusercontent.com/50439622/103340984-e91f7680-4ac8-11eb-995a-bf3253819534.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 52. toaster node operation

  Server의 Host명, Port, Name/PW 입력 이후 Capability connect 접속을 먼저 수행한다. <br>toaster model의 \<get\> 작업 선택 이후 RPC를 클릭하게 되면 Encoding 화면에 해당 toaster 관련 작업이 xml로 자동으로 설정되는 것을 확인할 수 있다.

  ![image](https://user-images.githubusercontent.com/50439622/103600832-b8699200-4f4b-11eb-9eec-2d20ae8afe74.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 53. toaster model RUN 수행

  위 그림과 같이 Custom RPC 선택 후 Run을 클릭하게 되면 **client(yang-explorer)는 server(OpenYuma)로 선택된 Operator(ex, \<get\>)설정을 가지고 오겠다는 메시지를 전달**하며(위의 예에서는 toasterManufacturer, toasterModelNumber, toasterStatus) **Server에서는 관련 메시지에 해당하는 데이터를 client에게 전달**하게 된다. (위의 예에서는 toasterManufacturer = Acme, Inc / toasterModelNumber = Super Toastamatic 2000 / toasterStatus = up) yang-explorer를 통해 toaster yang 모듈을 정상적으로 가져오는 것이 확인되었다.

  ![image](https://user-images.githubusercontent.com/50439622/103506953-5ce2ca00-4ea1-11eb-989c-09b4878880e6.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 54. libtoaster get Operator 확인

  이후 libtoaster(OpenYuma 기본 제공 yang 모듈)를 수정하였을 경우에도 정상적으로 가져오는지 확인이 필요하였다. Test를 위해 toaster.c 파일의 toasterManufacturer와 toasterModelNumber를 각각 수정하고 OpenYuma root 디렉토리 상에서 make / sudo make install 수행하여 compile 해준뒤 서버를 실행 하였다. 위의 그림과 같이 client는 수정된 데이터를 정상적으로 받는 것을 확인했다.

  > toaster.c 파일 경로 : /libtoaster/src

  **yangcli(OpenYuma) 통해 toaster module load 시킬 때 과정** : 1. create-subscription => 2. load toaster => 3. mgrload toaster => 4. get-locks => 5. create toaster => 6. save => 7. release-locks

  ※ yang-explorer에서 위와 같은 작업 수행을 하기가 어렵다. session이 연결 되고 하나의 작업이 수행되면 session이 자동으로 close 되어버려서 다음 작업을 진행할 수 가 없었다. 그렇기 때문에 3 ~ 6번 까지의 작업을 yang-explorer에서는 설정 할 수가 없다.(yang-explorer 관련 문서를 봐도 lock/unlock 등과 관련된 내용은 찾기가 힘듬. Custom RPC를 통해 xml encoding을 수동으로 작성하면 lock은 가능하지만 작업 이후 session이 종료되고, unlock 수행은 새 session에서 진행 되므로 해당 session에서는 lock 설정을 하지 않은 상태 이므로 error가 발생됨) 그래서 yangcli통해 toaster module load 시킨 이후 yang-explorer에서는 각 toaster 값 \<get\> 작업을 통해 가져오는 역할만을 테스트하였다.

  ![image](https://user-images.githubusercontent.com/50439622/103514509-1c3e7d00-4eb0-11eb-8425-b69b2df3850f.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 55. make_sil_dir 통한 toaster stub 파일

  위의 과정들은 make_sil_dir 통해 toaster 관련 stub 파일(.c, .h)을 받고 toaster.c 파일 수정 후(string 변경)에도 동일하게 정상적으로 \<get\>작업 수행됨을 확인 하였다. (단, make_sil_dir을 통해 받은 toaster stub 파일과 기본 제공 libtoaster의 src 파일의 코드 차이 때문에 실행과정에서 받아오는 데이터에 차이가 있음)

  > make_sil_dir 수행 경로 : <OpenYuma 디렉토리>/netconf

  

  ~~※notification 관련 yang-explorer docs를 확인해보면 현재 버전의 yang explorer에는 수신된 netconf 알림을 볼 수 있는 옵션이 없다고 한다. 일반적으로 응용 프로그램의 주 메뉴에 있는 클릭 가능한 알림 로그에 저장된다고 하는데 주 메뉴에 클릭가능한 알림 로그를 찾아보기 힘들다. 그래서 server에서 보내는 notification 알림을 확인해 볼 수 있는 방법을 찾을 수 없고, \<rpc-reply\>를 통해 ok 메시지만을 전달 받는다. 그 과정은 아래 **Toaster.yang 모듈 \<notification\> test**에 나와있다.~~

  ※ yang-explorer는 한번의 operation이 수행 되고 session이 종료된다. 여러 번의 operation을 수행해도 session이 종료되지 않게 하려면 별도의 제어 작업이 필요하다. (yang-explorer의 여러 파일을 찾아 보며 session-close 관련 동작을 어디서 수행하는지 확인해보고 있지만 확실하게 알아내기가 힘듬)

- **Toaster.yang 모듈 \<notification\> test**

  ![image](https://user-images.githubusercontent.com/50439622/103737308-49686800-5035-11eb-86f8-b0ddc85aa52b.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 56. toaster.yang model

  위 그림은 toaster.yang model의 **make-toast rpc node 의 일부분**이다. make-toast의 input node 안에는 **toasterDoneness**와 **toasterToastType** leaf node 가 존재한다. 

  > toaster.yang model 파일 경로 : <OpenYuma 디렉토리>/netconf/module/netconfcentral/

  

  ![image](https://user-images.githubusercontent.com/50439622/103735881-5cc60400-5032-11eb-8dbc-bc367942c325.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 57. toaster.yang notification test(1)

  yang-explorer의 Explorer Area에 toaster.yang 파일을 visible 상태로 업로드 하고, 각 하위 노드들을 확인하면 동일하게 make-toast의 input 노드에 toasterDoneness와 toasterToastType leaf node가 존재하는 것을 확인할 수 있다. toasterDoneness의 value를 4로 입력하고, toasterToastType의 value를 toast:frozen-waffle로 입력하고 RPC를 클릭하게 되면 server로 보낼 xml파일이 자동으로 만들어 지는 것을 확인할 수 있다.(value에 대한 정보는 toaser.yang 파일에서 확인할 수 있다.)

  

  ![image](https://user-images.githubusercontent.com/50439622/103737384-787ed980-5035-11eb-98a3-2a8a445a164b.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 58. toaster.yang Custom RPC

  여기서 주의해야 할 점은 RPC 클릭 이후 자동으로 만들어지는 XML 설정에 \<get\>, \<filter\>와 같은 불필요한 노드들이 섞여서 들어간다는 점이다. Custom RPC 클릭 이후 위 그림의 **네모박스 부분을 제거**해 주어야 한다. yang-explorer에서 자동으로 xml 파일을 만들때 rpc element이면 default로 \<get\>, \<filter\> node를 함께 추가해주는 것 같다.

  

  ![image](https://user-images.githubusercontent.com/50439622/103737508-b3810d00-5035-11eb-9665-b14b994bdd10.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 59. toaster.yang notification test(2)

  Custom RPC로 \<get\>, \<filter\> 노드를 제거해주고 Run을 클릭하면 위 그림과 같이 server에서는 \<make-toast> 데이터를 받고 client로 ok message를 \<rpc-reply\> element에 담아 정상적으로 보내는 것을 확인할 수 있다. 

  ※ server debug문을 보면 **session \<id\> closed** 가 마지막 줄에 출력된다. make-toast 수행 이후 약 4-50초 경과후 server는 client에게 toastDone notification 메시지를 보낸다.[(yangcli를 통한 notification 참고)](#85-startstop-making-toast-notification-test) 하지만, yang-explorer에서는 작업 수행이 완료됨과 동시에 session이 종료되어 subscription 동작상태를 확인하기가 힘들다.

  Cisco yang-explorer 관련 문서에 나와있는 notification 예문으로 테스트해본 결과 error 가 발생한다. [RFC 5277:NETCONF Event Notification](https://tools.ietf.org/html/rfc5277#section-2.1.1)에 **2.1.1.1 Usage Example** 부분을 확인해 보면 notification xml 에 대한 사용자 example이 명시되어 있다.

  ![image](https://user-images.githubusercontent.com/50439622/103749062-5b073b00-5048-11eb-9de5-8bfe4b262194.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 60. create-subscription

  Usage Example에 나와있는 create-subscription을 yang-explorer로 가져와 주었다.

![image](https://user-images.githubusercontent.com/50439622/103749208-90ac2400-5048-11eb-8c4f-08534f3df7f0.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 61. rpc-reply create-subscription

Run 실행 후 메시지를 확인해 보면 ok 메시지를 받은것을 확인할 수 있고, server debug문을 보면 subscription을 실행한 것을 확인할 수 있다.