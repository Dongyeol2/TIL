...
처음 OpenYuma installation guide로 install을 진행했을때, 특정OpenYuma Package의 .deb 파일들의 install이 제대로 되지 않는 문제점이 있었다. 관련 doc 을 찾아보아도 해결방안을 찾기 힘들어 OpenYuma대신 YumaPro에서 진행했던 방식대로 Server, Client 관련 Library들을 다운로드 받았으며, https://github.com/OpenClovis/OpenYuma[링크] 에서 Open source 코드 다운로드 받은 후 make / sudo make install 진행 했다. 
