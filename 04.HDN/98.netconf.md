...
9. [IETF 표준 Module load test](#ietf-표준-module-load-test)<br>9.1 [OpenYuma 내에서 사용할 서버계측을 만드는데 필요한 단계](#91-openyuma-내에서-사용할-서버계측을-만드는데-필요한-단계)<br>9.2 [기존 IETF 모듈을 통한 서버 계측 구성](#92-기존-ietf-모듈을-통한-서버-계측-구성)
...
- YANG 모듈 데이터 모델 정의를 **생성**하거나 **기존 YANG 모듈을 사용**한다. (개발 시간이 많지 않기 때문에 우선적으로 기존 YANG 모듈 사용에 중점)

  ※ **yangdump 프로그램** : YANG 모듈의 유효성을 검사하고 오류가 없는지 확인 한다. 또한 모든 경고를 조사하여 데이터 모델링 버그를 나타내는지 여부를 확인해야 한다.(SIL이 수행되면 yangdump도 함께 수행된다고 생각하면 될 것같다.)

- **$YUMA_HOME**[5.1.1 Searching OpenYuma Roots](#511-searching-openyuma-roots) 환경변수가 정의되어 있고 OpenYuma 개발 트리를 가리키는지 확인해야 한다.

- SIL 개발 하위 트리를 만들어야 한다.

  **/usr/bin** 디렉토리에 설치된 **make_sil_dir** 스크립트를 사용하여 디렉토리 구조와 Makefile을 생성한다. 이 단계에서 **yangdump를 호출하여 SIL의 초기 .H, .C 파일을 생성**하게 된다.
...
- **Compile code**
  ...
  SIL **'src' 디렉토리**에서 **'make'** 명령을 사용한다. SIL **'lib' 디렉토리**에 라이브러리 파일이 생성되어야 한다.

- **netconfd 서버**에서 사용할 수 있도록 SIL 라이브러리를 설치한다.

  SIL **'src'디렉토리**에서 **'make install' 명령**을 사용한다.

- **netconfd Server 실행**

- **새로운 Module Load**

  재부팅 할 때마다 모듈을 로드해야하는 경우 구성 파일에 'load' 명령을 추가해야 한다. (yangcli에서 
  **'load <Module 명>'**)
