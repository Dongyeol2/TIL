...
10. [yang-explorer(cisco yang data model)](#10-yang-explorercisco-yang-data-model)<br>10.1 [Installation](#101-installation)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;10.1.1 [Prerequisite(전제 조건)](#1011-prerequisite전제-조건)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;10.1.2 [Download and install](#1012-download-and-install)<br>10.2 [Running YangExplorer](#102-running-yangexplorer)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;10.2.1 [Running with localhost](#1021-running-with-localhost)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;10.2.2 [Running with ip-address (Shared Server)](#1022-running-with-ip-address-shared-server)<br>10.3 [User Guide](#103-user-guide)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;10.3.1 [Work Flow](#1031-work-flow)<br>
...
- **YANG 데이터 모델 파일 대량 로드**
  ...
  cisco vendor 관련 모듈이 아닌 ietf 표준 모듈을 위와 같은 방식으로 업로드하려고 했지만 compile error가 남.
...
- **Toaster.yang(OpenYuma sample yang model)을 이용한 yang-explorer test**

    ietf 표준 모듈을 통해 test를 진행해보려 했지만, netconf server가 실제 장비에 물려있는 것이 아니기 때문에 해당 설정 정보를 제대로 가져오지 못했다. 수행은 정상적으로 되지만, (데이터값을 찾을 수 없기때문에 가져오지 못하는 현상) 그래서 yang-explorer에서 load한 모듈을 가지고 server에서 데이터값을 가지고 오는지 확인하기 위해 OpenYuma 에서 기본으로 제공하는 toaster.yang 파일을 가지고 우선적으로 test해보기로 하였다.
  ...
  yang-explorer(cisco application)을 통해 yang model 사용하기 위해 upload를 먼저 해주어야한다. toaster.yang 모델의 경우 기존 yang-explorer 기본 제공 model에는 존재하지 않기 때문에 openyuma/netconf/module/netconfcentral 에서 찾아 upload 해주어야 한다. (server <-> client 간에는 동일한 yang 파일이 존재해야함.)
  ...
  toaster.yang 파일 업로드 이후 compiling 시간을 기다린 후에 위 그림과 같이 toaster 모듈이 workspace에 적재된 것을 확인할 수 있다. 이후 하단 Subscribe를 클릭하게 되면 Explorer 화면에 toaster 모듈의 작업이 가능해진다.
  ...
  Server의 Host명, Port, Name/PW 입력 이후 Capability connect 접속을 먼저 수행한다. <br>toaster model의 \<get\> 작업 선택 이후 RPC를 클릭하게 되면 Encoding 화면에 toaster 관련 작업 node들이 설정되는 것을 확인할 수 있다.
  ...
  위 그림과 같이 Custom RPC 선택 후 Run을 클릭하게 되면 client(yang-explorer)는 server(OpenYuma)로 선택된 Operator(ex, \<get\>)설정을 가지고 오겠다는 메시지를 전달하며(위의 예에서는 toasterManufacturer, toasterModelNumber, toasterStatus) Server에서는 관련 메시지에 해당하는 데이터를 client에게 전달하게 된다. (위의 예에서는 toasterManufacturer = Acme, Inc / toasterModelNumber = Super Toastamatic 2000 / toasterStatus = up)
  
  yang-explorer를 통해
  
   toaster yang 모듈을 정상적으로 가져오는 것이 확인되었다. 이후 libtoaster(OpenYuma 기본 제공 yang 모듈)를 수정하였을 경우에도 정상적으로 가져오는지 확인이 필요하였다. 
  ...
  Test를 위해 toaster.c 파일의 toasterManufacturer와 toasterModelNumber를 각각 수정하고 OpenYuma root 디렉토리 상에서 make / sudo make install 수행하여 compile 해준뒤 서버를 실행 하였다. 
  ...
  **yangcli(OpenYuma) 통해 toaster module load 시킬 때 과정** : 1. create-subscription => 2. load toaster => 3. mgrload toaster => 4. get-locks => 5. create toaster => 6. save => 7. release-locks
  ...
  make_sil_dir 통해 toaster 관련 stub 파일을 받고 toaster.c 파일 수정 후 정상적으로 \<get\>작업 수행됨을 확인 하였다.
  ...
  ※ yang-explorer는 한번의 operation이 수행 되고 session이 종료되는것이 특징이다. 여러 번의 operation을 수행해도 session이 종료되지 않게 하려면 별도의 제어 작업이 필요하다.
