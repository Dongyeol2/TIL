...
# 1. Netconf(Network C onfiguration Protocol)
...
yang suitre는 Docker 기반의 YANG모델 (RFC 6020, RFC 7950) 및 NETCONF(RFC 6241)과 관련된 도구 세트이다. 
...
3. Run docker-compose
   ...
   ※ sudo docker-composer up 수행 전 depandencies library 설치

   ```shell
   sudo apt-get install git openssh-client iputils-ping python3.6 python3-pip sqlite3 vim
   ```

   ※ docker-compose up 수행 시 'ERROR: Couldn't connect to Docker daemon - you might need to run 'docker-machine start default' 발생할 경우
...
4. https://localhost 접속
   ...
   초기 admin id : admin / password : supoeruser
