...
10. [yang-explorer(cisco yang data model)](#10-yang-explorercisco-yang-data-model)<br>10.1 [Installation](#101-installation)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;10.1.1 [Prerequisite(전제 조건)](#1011-prerequisite전제-조건)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;10.1.2 [Download and install](#1012-download-and-install)<br>10.2 [Running YangExplorer](#102-running-yangexplorer)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;10.2.1 [Running with localhost](#1021-running-with-localhost)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;10.2.2 [Running with ip-address (Shared Server)](#1022-running-with-ip-address-shared-server)<br>10.3 [User Guide](#103-user-guide)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;10.3.1 [Work Flow(Login)](#1031-work-flowlogin)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;10.3.2 [Work Flow(Adding / Deleting Yang Models)](#1032-work-flowadding--deleting-yang-models)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;10.3.3 [Work Flow(Yang Explorer 애플리케이션 사용)](#1033-work-flowyang-explorer-애플리케이션-사용)<br>10.4 [Toaster.yang(OpenYuma sample yang model)을 이용한 yang-explorer test](#104-toaster.yangopenyuma-sample-yang-model을-이용한-yang-explorer-test)
...
- **Toaster.yang 모듈 \<get\> test**
  ...
  **yangcli(OpenYuma) 통해 toaster module load 시킬 때 과정** : 1. create-subscription => 2. load toaster => 3. mgrload toaster => 4. get-locks => 5. create toaster => 6. save => 7. release-locks
  ...
  위의 과정들은 make_sil_dir 통해 toaster 관련 stub 파일(.c, .h)을 받고 toaster.c 파일 수정 후(string 변경)에도 동일하게 정상적으로 \<get\>작업 수행됨을 확인 하였다.

  > make_sil_dir 수행 경로 : <OpenYuma 디렉토리>/netconf

  ※notification 관련 yang-explorer docs를 확인해보면 현재 버전의 yang explorer에는 수신된 netconf 알림을 볼 수 있는 옵션이 없다고 한다. 일반적으로 응용 프로그램의 주 메뉴에 있는 클릭 가능한 알림 로그에 저장된다고 하는데 주 메뉴에 클릭가능한 알림 로그를 찾아보기 힘들다. 그래서 server에서 보내는 notification 알림을 확인해 볼 수 있는 방법을 찾을 수 없고, \<rpc-reply\>를 통해 ok 메시지만을 전달 받는다.

  ※ yang-explorer는 한번의 operation이 수행 되고 session이 종료된다. 여러 번의 operation을 수행해도 session이 종료되지 않게 하려면 별도의 제어 작업이 필요하다.
