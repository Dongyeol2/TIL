...
- YANG 데이터 모델링 언어[RFC6020]는 Operations 및 Content 계층을 망라하여 NETCONF 데이터 모델 및 프로토콜 운영을 명시하기 위해 개발되었다.
  ...
  **1. Configuration Database**
  ...
  ![image](https://user-images.githubusercontent.com/50439622/101708268-3f078c80-3ad0-11eb-8bc0-a8431dcbf565.png)
...
**3.1.3 Configuration and State Data**
...
**3.1.4 Reusable Node Groups (grouping)**
...
**3.1.6 Extending Data Models (augment)**
...
**3.1.8 Notification Definitions**
...
## How to Configure the NETCONF Protocol(Cisco)
...
### Configuring NETCONF Options (CISCO IOS 기반)
...
## YumaPro SDK INSTALLATION

원문 출처 : https://www.yumaworks.com/support/download-documentation/



### 1. Installing YumaPro SDK

ubuntu 18.04 LTS 에서 testing



### 2. External Package Needed by the Server
...
### 3. External Packages Needed by the Client
...
Ubuntu 18.04 AMD/Intel 64-bit : [yumapro-sdk-basic-19.10-12.u1804.amd64.deb](https://www.yumaworks.com/download/yumapro-sdk-basic-19-10-12-u1804-amd64-deb/)
...
- **Server Startup Issues**
  ...
  이전에 netconfd-pro 서버를 실행한 적이 있고 아래 메시지가 표시되는 경우 서버가 다른 프로세스에서 계속 실행중이거나 마지막으로 실행되었을 때 완전히 종료되지 않았음. 
  실행중인 다른 서버를 중지하거나 메시지의 지침에 따라 이전 실행에서 남은 파일 정리 이후 재수행.
...
- netconfdpro를 백그라운드에서 실행하고 모든 출력을 로그 파일에 저장하는 것이 가장 편리
...
- **Server Setup**
  ...
  서버에서 인증서 작업을 위한 두개의 디렉토리를 생성하고 generate-keys.sh 스크립트 파일을 복사.
...
### 9. Enable Notification Delivery
...
### 11. Enable the Toaster

```
dylee@localhost> make-toast
...
### 12. Get the Toaster State Information(GET Test)
...
'xget' 명령은 \<get\> 작업에 대한 상위 XPath 필터 처리기.

```shell
dylee@localhost> xget /toster
```

두 명령어 모두 아래의 결과가 반환될 것임.
...
### 13. Start/Stop Making Toast (notification Test)

Toster 활성되었으므로 'make-toast' 명령이 작동된다.
...
-> 이전 명령을 반복(Control-P는 이전 명령을 호출해야 함)
...
- **Default Value Filtering**
  ...
  1. **report-all-tagged** : no-filtering -- 서버에 의해 기본값으로 간주되기 떄문에 서버가 억제할 수 있는 모든 노드를 반환한다. (기본적으로 설정된 노드에는 'default' XML 속성이 추가됨)
...
- **Special Retrieval Operation**
  ...
  netconfd-pro에서 지원하는 두 가지 사용자 지정 검색작업
...
- **Non-Volatile Storage**
  ...
  ynagcli-pro 프로그램에는 편집 작업 후에 사용되는 **'save' command**가 있으며, 이 명령은 자동으로 올바른 프로톸로 작업을 실행하여 편집을 완료하고 변경 사항을 non-volatile 저장소에 저장한다.

  startup database는 netconf-pro 서버에서 구성할 수 있다. --with-startup configuration parameter는 startup database의 사용여부를 제어한다.
...
YumaPro 프로그램은 시스템 환경변수를 활용하여 프로그림의 구성 및 작동을 사용자 정의 하고 단순화한다.<br />이러한 환경변수는 일번적으로 파일 검색 결로 또는 기본 디렉토리 위치를 지정한다.
...
## YumaPro Developmet Manual

관련 docs : https://www.yumaworks.com/pub/docs/latest/pdf/yumapro-dev-manual.pdf<br />관련 Open sourcee : https://github.com/OpenClovis/OpenYuma

Open source zip파일 다운로드 받은 후 압축 해제
...
**3.1.3 What is a OpenYuma Root?**

![image](https://user-images.githubusercontent.com/50439622/101724682-843cb600-3af2-11eb-8786-381659301d47.png)
...
**3.1.4 Searching OpenYuma Roots**

![image](https://user-images.githubusercontent.com/50439622/101725194-89e6cb80-3af3-11eb-8f10-fe1bf9cff7c7.png)
...
1. YANG 모듈 데이터 모델 정의를 생성하거나, 기존 YANG 모듈을 사용<br/>yangdump 프로그램으로 YANG 모듈의 유효성을 검사하고 오류가 없는지 확인. 또한 모든 경고를 조사하여 데이터 모델링 버그를 나타내는지 여부를 확인해야한다. Ex) toaster.yang

2. **&YUMA_HOME** 환경변수가 정의되어 있고 OpenYuma 개발 트리를 가리키는지 확인

3. **SIL 개발 subtree 생성**<br/>**/usr/bin** 디렉토리에 설치된 **make_sil_dir** 스크립트를 사용하여 디렉토리 구조와 Makefile을 생성. 이 단계에서 **yangdump를 호출하여 SIL 초기 .h파일 및 .c 파일을 생성**한다. Ex) mydir> make_sil_dir -split test
4. 텍스트 편집기를 사용하여 각 object, RPC method, notification에 대한 장치 별 instrumenation을 채운다. Ex) test/src/u_test.c<br/>거의 모든 NETCONF 관련 코드는 중앙 스택에서 처리되거나 자동으로 생성된다. 따라서 이 코드는 YANG 데이터 모델의 의미를 구현한다.
5. **Compile Code**<br/>SIL 'src' 디렉토리에서 'make' 명령을 사용. SIL 'lib' 디렉토리에 라이브러리 파일이 생성되어야 한다. Ex) mydir/test/stc> make
6. **netconfd 서버에서 사용할 수 있도록 SIL 라이브러리 설치**<br/>SIL 'src' 디렉토리에서 'make install'명령을 사용 Ex) sudo make install
7. **netconfd 서버 실행**
8. **새 모듈 로드**<br/>재부팅 할 때마다 모듈을 로드해야하는 경우 구성파일에 'load' 명령어 추가해야한다. Ex) load test
...
![image](https://user-images.githubusercontent.com/50439622/101874479-4d3ad300-3bcc-11eb-8a21-b8bc0e7245e2.png)
...
![image](https://user-images.githubusercontent.com/50439622/101875861-bae7fe80-3bce-11eb-9ce6-c4db32604d16.png)

YumaPro는 YANG 소스 모듈을 직접 사용하여 서버내에서 NETCONF 프로토콜 작업을 자동으로 구현한다. 모든 YumaPro 프로그램에서 동일한 YANG parser를 사용한다. 'ncx' 소스 디렉토리(libncx.so)에 있음(OpenYuma-master/netconf/target/lib). 응용프로그램에서 설정하는 여러가지 구문분석 모드가 있다.<br/> 'server mode'에서 'description'과 'reference'와 같은 설명문은 입력 시 폐기 된다. 장치가 읽을 수 있는 명령문만 저장됨. 가능한 모든 데이터베이스 유효성 검사, 필터링, processing, 초기화, NV-storage, 오류 처리는 이러한 machine 판독 가능 문을 기반으로 수행된다. <br/>YANG 파일은 직접 또는 간접적으로 변경될 수 있으며 여기에 지정된 새로운 또는 변경된 default 값이 있다. 또한 range statement, patterns, XPath expressions, 기타 모든 기계 판독 가능 문은 자동으로 처리되므로 YANG 문 자체는 서버 소스코드와 같다.

...

**3.3.6 Server Message Flows**

![image](https://user-images.githubusercontent.com/50439622/102042780-10fab300-3e16-11eb-9753-fd9fc2bcb192.png)
...
모든 NETCONF 및 YANG 프로토콜 작업 세부사항은 netconfd-pro 서버 내에서 자동으로 처리된다. 모든 데이터베이스 잠금 및 편집도 서버에서 처리한다. 각 서버 요청을 처리하거나 알림을 생성하기 위해 모듈별 계측 코드에 대해 처리 모델의 여러 지점에서 사용할 수 있는 콜백 함수가 있다. 'description statement' 의미 체계를 제외한 모든것이 일반적으로 처리된다.

데이터 모델 의미 체계와 관련된 서버 계측 stub 파일은 yangdump-pro 프로그램을 사용하여 자동으로 생성된다. 개발자는 각 YANG 데이터 모델이 나타내는 네트워킹 장치 동작을 활성화하기 위해 서버 콜백함수를 채운다.

**3.3.7 Main ncxserver Loop**

![image](https://user-images.githubusercontent.com/50439622/102043453-c8dc9000-3e17-11eb-986c-64ddbff820ed.png)

ncxserver 루프는 거의 작동하지 않으며 자체 SSH 서버가 있는 임베디드 서버에서 교체되도록 설계되었다.

- 클라이언트가 SSH 세션을 시작하도록 요청하면 netconf-subsystem-pro 프로그램의 인스턴스에 SSH 채널이 설정된다.
- netconf-subsystem-pro 프로그램은 local socket(/tmp/ncxserver.sock)을 열고 독점 < ncxconnect > 메시지를 netconfd-pro 서버로 보낸다. netconfd-pro 서버는 선택 루프를 사용하여 이 local socket에서 수신 대기한다.(agt_ncxserver.c 에서)
- netconfd-pro에서 유효한 < ncxconnect > 메시지를 받으면 새 NETCONF 세션이 생성된다.
- < ncxconnect > 메시지를 보낸 후 netconf-subsystem-pro 프로그램은 'transfer mode'로 들어가서 SSH 채널의 입력을 netconfd-pro 서버로 전달하고 netconfd-pro 서버의 출력을 SSH로 전달한다.
...
![image](https://user-images.githubusercontent.com/50439622/102045499-91241700-3e1c-11eb-9e03-7ed987a21c48.png)

- Top Level : 최상위 레벨 수신 메시지는 서버 메시지 처리 설계에 고정 배선되지 않고 등록된다. agt-ncxserver 모듈은 netconf-subsystem-pro의 < ncxconnect > 메시지를 수락하고, agt_rpc 모듈은 NETCONF < rpc > 메시지를 수락한다. top_register_node 함수를 사용하여 서버에서 추가 메시지를 지원할 수 있다.
...
YumaPro 소프트웨어 개발 환경에서 사용되는 몇가지 구성요소

- gcc compiler와 linker
- ldconfig 및 install programs
- GNU make 프로그램
- bash와 같은 shell 프로그램
- YumaPro 개발 트리 : $YUMAPRO_HOME 환경 변수로 지정된 YumaPro 코드를 포함하는 소스 트리
- SIL 개발 트리 : 서버 계측 코드가 포함된 소스 트리

다음 외부 프로그램은 YumaPro에서 사용하며 사전 설치 해야한다.

- opensshd (netconfd-pro 에 필요)
  - SSH2 서버 코드는 netconfd-pro와 연결되지 않는다. 대신 netconf-subsystem-pro 프로그램이 호출되고 이 SSH2 하위 시스템에서 netconfd-pro 서버로 로컬 연결이 설정된다.

다음 프로그램은 YumaPro tools의 일부이며, 설치가 필요

- netconf-subsystem-pro (netconfd-pro에 필요)
  - 'netconf' 하위 시스템에 대한 새 SSH2 연결을 시도할 때 sshd에서 호출하는 thin 클라이언트 하위 프로그램.
    - 이 프로그램은 netconfd-pro 서버와 통신하기 위해 독점 < ncxconnect > 메시지를 사용하는 AF_LOCAL 소켓을 사용한다.
    - netconfd-pro 서버와 연결을 설정한 후 이 프로그램은 sshd와 netconfd-pro 간에 SSH2 NETCONF 채널 데이터를 전송한다.

다음 프로그램은 YumaPro Tools의 일부이며 일반적으로 YumaPro 개발 트리에서 찾을 수 있다.

- netconfd-pro
  - 모든 프로토콜 작업을 처리하는 NETCONF 서버
    - agt_ncxserver 구성 요소는 지정된 소켓( /tmp/ncxserver.sock )에서 < ncxconnect > 메시지를 수신한다. 유효하지 않은 메시지가 수신되면 연결이 끊어지며, 유효한 메시지 수신 시 netconf-subsystem-pro는 NETCONF 채널 데이터를 netconfd-pro 서버로 전달하기 시작한다. 첫번째 메시지는 < hello > PDU 메시지.

다음 외부 라이브러리는 YumaPro에서 사용되며 사전 설치해야한다.

- ncurses ( yangcli-pro에 필요)
  - libtecla에 필요한 문자 처리 라이브러리. yangcli-pro 내에서 사용
- libc (또는 glibc, 모든 응용프로그램에 필요)
  - unix 시스템 라이브러리
- libssh2 ( yangcli-pro에 필요)
  - yangcli-pro에서 사용하는 SSH2 클라이언트 라이브러리
- libxml2 ( 모든 응용 프로그램에 필요)
...
**SIL callback 코드는 netconfd-pro 프로세스에서 실핸된다.**
...
- 외부 모듈에 대한 \<augment\> 문
  ...
  ![image](https://user-images.githubusercontent.com/50439622/102061103-621a9f00-3e36-11eb-9b66-5ca49a276c79.png)

**make_sil_dir_pro** 스크립트는 단일 YANG 모듈에 대한 .C 및 .H 파일 코드 stub을 생성하는데 사용할 수 있다. 이 스크립트 는 **yangdump-sdk** 프로그램을 사용하여 이러한 파일을 생성한다. 이 모드에서 SIL 코드는 대상 모듈의 정의에 대해서만 생성된다. 대상 모듈 (또는 다른 모듈)의 외부 기능 보강 문은 SIL 코드 생성에 영향을 주지 않는다.

단일 모듈 용 SIL 라이브러리는 **--module** 매개 변수를 사용하여 서버에 로드된다.

**make_sil_bundle** 스크립트는 YANG 모듈 세트에 대한 .C 및 .H 파일 코드 stub을 생성하는데 사용할 수 있다. 이 스크립트는 **yangdump-sdk** 프로그램을 사용하여 이러한 파일을 생성하지만 **--sil-bundle** 매개변수를 설정한다. 이 모드에서는 대상 모듈의 정의에 대해 SIL 코드가 생성된다. 함께 로드된 모든 모듈의 외부 확장 명령문은 SIL 코드 생성에 영향을 준다.

SIL 번들용 SIL 라이브러리는 --bundle 매개변수를 사용하여 서버에 로드된다.
...
기본적으로 SIL 바이너리는 /usr/lib/yumapro 디렉토리에 설치된다.
...
```
>make_sil_dir --split <파일명>
```

![image](https://user-images.githubusercontent.com/50439622/102151487-5e2e6180-3eb6-11eb-883d-423f6daa4634.png)



### :high_brightness: Basic YANG Module Structure

![image](https://user-images.githubusercontent.com/50439622/102177751-808ca300-3ee7-11eb-9f22-259a4dd6bbce.png)
...
### :high_brightness: Identifiers in YANG Modules
...
### :high_brightness: YANG Module Revisions

모든 YANG 모듈에는 개정판이 있다.

- 사용된 가장 최근 개정판 (data string)
  ...
  ```
   ...
   revision statements가 없는 경우 모듈의 revision은 'NULL'을 갖는다.

- 외부 모듈의 명시적 개정을 가져올 수 있다.
...
### :high_brightness: YANG Submodules

YANG 모듈은 "submodules"이라고 하는 여러 파일로 분할 할 수 있다.
...
## OpenYuma OpenSource Installation
...
### 1. External Package Needed by the Server

```shell
mydir> sudo apt-get install libxml2-dev
```

일부 XML 구문 분석 기능에 필요. Linux 내에 default로 설치되어 있지만, openyuma를 빌드하려면 DEB 기반의 libxml2-dev라는 개발자 패키지가 필요함.
...
### 2. External Packages Needed by the Client
