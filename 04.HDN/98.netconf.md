...
- **Toaster.yang 모듈 \<notification\> test**
  ...
  ※ server debug문을 보면 **session \<id\> closed** 가 마지막 줄에 출력된다. make-toast 수행 이후 약 4-50초 경과후 server는 client에게 toastDone notification 메시지를 보낸다.[(yangcli를 통한 notification 참고)](#85-startstop-making-toast-notification-test) 하지만, yang-explorer에서는 작업 수행이 완료됨과 동시에 session이 종료되며, notification log를 확인할 수 있는 옵션을 제공하는지 확인이 힘들어 ok 메시지를 받는 것으로 정상동작되었다고 간주한다.
