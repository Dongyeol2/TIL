...
mydir> **sudo dpkg -i yumapro-sdk-basic-19.10-12.u1804.amd64.deb**
...
NETCONF 서버를 시작하려면 sshd 서버가 실행 중인지 확인하고 구성이 **/etc/ssh/sshd_config**에 포함되어 있는지 확인
...
- **Installing SSH keys**
  ...
  ```
  > ssh-copy-id <your_username>@<server's IP address>
  ```
...
- netconfd-pro 서버에 동시에 연결할 수 있는 동시 SSH 세션 수는 제한될 수 있다.
...
### 12. Get the Toaster State Information(GET 예제)
...
### 13. Start/Stop Making Toast (notification 예제)
