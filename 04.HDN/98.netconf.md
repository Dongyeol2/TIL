...
관련 docs : https://www.cisco.com/c/ko_kr/support/docs/storage-networking/management/200933-YANG-NETCONF-Configuration-Validation.html <br>관련 Opensource link : https://github.com/CiscoDevNet/yang-explorer
...
- **Toaster.yang(OpenYuma sample yang model)을 이용한 yang-explorer test**
  ...
  ![image-20201230180625594](C:\Users\hdn\AppData\Roaming\Typora\typora-user-images\image-20201230180625594.png)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;그림 53. toaster model RUN 수행
  ...
  yang-explorer를 통해 toaster yang 모듈을 정상적으로 가져오는 것이 확인되었다. 이후 libtoaster(OpenYuma 기본 제공 yang 모듈)를 수정하였을 경우에도 정상적으로 가져오는지 확인이 필요하였다. 
  ...
  **yangcli(OpenYuma) 통해 toaster module load 시킬 때 과정** : 1. create-subscription => 2. load toaster => 3. mgrload toaster => 4. get-locks => 5. create toaster => 6. save => 7. release-locks
  ...
  yang-explorer에서 위와 같은 작업 수행을 하기가 어렵다. session이 연결 되고 하나의 작업이 수행되면 session이 자동으로 close 되어버려서 다음 작업을 진행할 수 가 없었다. 3 ~ 6번 까지의 작업을 yang-explorer에서는 설정 할 수가 없다.(yang-explorer 관련 문서를 봐도 lock/unlock 등과 관련된 내용은 찾기가 힘듬. Custom RPC를 통해 xml encodig을 수동으로 설정하면 lock은 가능하지만 작업 이후 session이 종료되고, unlock 수행은 다른 session에서 진행 되므로 해당 session에서는 lock 설정을 하지 않은 상태 이므로 error가 발생됨) 그래서 yangcli통해 toaster module load 시킨 이후 yang-explorer에서는 각 toaster 값 \<get\> 작업을 통해 가져오는 역할만을 테스트하였다.
